...
publishes a genesis anyone can verify against this source tree. **What it has actually carried is a
READING, never a property of this page** - so this page states none. The `jobs` module's history is
one query away, and it does not go stale the way a sentence does:

```
curl -s http://api.dendranetwork.com:1317/dendra/jobs/v1/job
curl -s "http://api.dendranetwork.com:26657/tx_search?query=%22message.module%3D%27jobs%27%22&per_page=1"
```

**Two floors do stand in this path, and they are independent - but which side of either one the network
sits on is a READING, never a property this page can state.** The first is applied when a job is opened:
`chain/x/jobs/keeper/pool_freeze.go::enoughEligibleMinersToVerify` refuses a job that could never be
verified, and it needs `audit_min_quorum` + 1 miners eligible at that block, because the jury excludes
the miner under audit. The second is the audit draw's own: while `latest_contributors` stays under
`committee_min_vrf_contributors`, no committee is drawn for any job -
`dendrad query jobs committee-seed-health -o json` returns both numbers side by side. Only a **bonded
validator** can raise that second reading, by anchoring a VRF key
(`tx jobs register-validator-vrf-key`); registering as a miner does not move it. Below that floor the
beacon runs on its legacy fallback, so the anti-grinding property is **not** in force - and the chain
says so in its own logs at every block.

**This page quotes no count, and no comparison either.** A number written into a document is wrong at
the next registration - and a statement about which side of a floor the network sits on goes stale the
same way, while looking safer than a number. Read them from the chain (`/dendra/jobs/v1/params`,
`/dendra/jobs/v1/miner`, `/dendra/jobs/v1/committee_seed_health`); when the open-job refusal fires, it
names the shortfall in the transaction's error.
...
retained, not cancelled — and cannot be released while either floor is unmet: the jury floor, or the audit draw's contributor floor.
...
| **The served genesis matches the digest served beside it — and the one this repository pins.** Digest and genesis come from the same host and the same port, so that comparison alone catches a **truncated or corrupted download** and nothing more: whoever serves a different genesis serves the matching digest with it. The stronger check needs the digest from a place that host does not control, and [`docker/GENESIS_SHA256`](docker/GENESIS_SHA256) is that place: it reaches you through the repository host, and `deploy/join.sh` refuses a served genesis that differs from it. Two hosts, one operator — this rules out a forged genesis host, not a forged operator. | `curl -s -o g.json http://api.dendranetwork.com:8088/genesis.json && sha256sum g.json` — compare with `GENESIS_SHA256` in `http://api.dendranetwork.com:8088/network-info.txt` **and** with line 1 of `docker/GENESIS_SHA256` in your clone (`<chain_id>=<sha256>`); all three must agree. If the clone disagrees and the network was relaunched, `git fetch origin && git reset --hard origin/main` first. |
| **Fault tolerance is zero, and a registry count is not a validator set.** The staking registry can list more validators than are actually securing the chain: one that is **jailed** sits there with its stake, produces no blocks and contributes nothing to the randomness beacon. Only the **active** set matters, and losing it stops the chain. Read that set from the chain rather than from this page — the query is in the right-hand column, and the answer changes. This is not a hypothetical either: on the previous chain validators were jailed for downtime and lost stake for it, which is why [`deploy/README.md`](deploy/README.md#becoming-a-validator-read-this-first) states the cost of the role before you bond. | Active set: `curl -s http://api.dendranetwork.com:26657/validators` — read `result.total` and compare the `voting_power` values. Registry: `curl -s http://api.dendranetwork.com:1317/cosmos/staking/v1beta1/validators` — read `status` and `jailed` on each. **The two numbers differ, and the difference is the point**; anything not `BOND_STATUS_BONDED` with `jailed: false` is securing nothing. |
...
Two different things live below, and running the first one **breaks** the second. Pick the one you
want.
...
A miner needs **no inbound port**: it dials out to the relay, the RPC and the faucet, so a connection
behind carrier-grade NAT qualifies.

### Or run a private stack of your own, on your machine
...
> ⚠️ **Do not run both.** This stack binds `26657` and `26656`, the same ports the joining kit needs,
> and it uses the same `chain-id` (`dendra`) as the public network while sharing no history with it.
> Docker will not warn you: the two run under different project names, so the kit's "a node is already
> running" check does not see this one and `deploy/join.sh` fails on a port that nothing names. If you
> started this stack and then want to join, `docker compose down` first.
...
contributor floor that defers every audit draw while it is unmet — and with it, the retained earnings of every
miner.
