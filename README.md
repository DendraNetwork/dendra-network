...
publishes a genesis anyone can verify against this source tree. Since its most recent relaunch it serves
**nothing** - and the measured reason sits upstream of every floor below: **no job has ever been opened
on it.** The whole transaction history of the `jobs` module carries no `MsgOpenJob`; `tx_search` on
`message.module='jobs'` lists what the chain has actually carried. Sampling, committee draw, jury and
verdict are all downstream of a job existing, so none of them has ever had an input to act on.
...
| **The served genesis matches the digest served beside it.** Both come from the same host and the same port, so this catches a **truncated or corrupted download** — it does **not** show the file was not substituted: whoever serves a different genesis serves the matching digest with it. That stronger guarantee needs the digest from a place this host does not control, and no such place is published yet. | `curl -s -o g.json http://api.dendranetwork.com:8088/genesis.json && sha256sum g.json` — compare with `GENESIS_SHA256` in `http://api.dendranetwork.com:8088/network-info.txt` |
