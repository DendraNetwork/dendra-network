...
**nothing**: the registered miners are below the floor the chain enforces, so it refuses to open a
job at all. That floor is `audit_min_quorum` + 1 miners eligible at the block, because the jury excludes
the miner under audit; this page quotes neither number, since a count written into a document is wrong at
the next registration — read both from the chain (`/dendra/jobs/v1/params`, `/dendra/jobs/v1/miner`), and
the refusal itself names the shortfall in the transaction's error.

**Two floors block this network, and they are independent.** The first is the one above: registered miners
below `audit_min_quorum` + 1, so no job can be opened at all. The second is the audit draw's own: while
`latest_contributors` stays under `committee_min_vrf_contributors`, no committee is drawn for any job —
`dendrad query jobs committee-seed-health -o json` returns both numbers side by side. Only a **bonded
validator** can raise the first of those readings, by anchoring a VRF key
(`tx jobs register-validator-vrf-key`); registering as a miner does not move it. Below that floor the
beacon runs on its legacy fallback, so the anti-grinding property is **not** in force — and the chain says
so in its own logs at every block. Until both floors are cleared, no job is opened, none is checked,
