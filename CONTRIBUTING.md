...
project-operated, and **no job has ever been opened on it** - a fact upstream of every floor below - so
nothing is served and none is verified on-chain. Do not take a number from this page, and do not take a
comparison either - read it:
...
concludes only when `audit_min_quorum` jurors vote and the jury excludes the miner under audit, so it
needs one more registered miner than that quorum. Read that quorum and `audit_sample_bps` from the chain
rather than from any page: the first sets how many must vote, the second what share of jobs the lottery
draws, and a drawn job that cannot conclude keeps its fee retained.
