...
> On a machine that has no Docker yet — or on Windows — start one step earlier: `deploy/install.sh` (Ubuntu, Debian, HiveOS) or `deploy/install.ps1` (Windows, WSL 2) prepares the host and then runs `join.sh` itself. Download, read, run; nothing happens without `--yes`.
...
> beacon carries at least `committee_min_vrf_contributors` contributors, and a contributor is a
> **validator**, not a miner. Whether it does right now is a reading:
> `curl -s http://api.dendranetwork.com:1317/dendra/jobs/v1/committee_seed_health`. Mining does
> not unlock mining pay, and a second validator unlocks only **part** of it: with `audit_sample_bps` at
> `5000` the lottery selects **one job in two**, and a selected job releases nothing until an audit can
> **conclude** — `audit_min_quorum` (4) jurors voting, the primary excluded, i.e. **five registered
> miners**. The beacon frees the unsampled half; the jury frees the rest. Read [`../README.md`](../README.md) before you budget
...
> | `GET list` — the queue that tells you a job awaits you | **nothing.** Gated, this is what leaves a joined miner registered and idle: no job, no error, nothing to search for. A secret here would hide only what the chain already publishes anonymously (`assigned_committee/<jobId>`). |
