...
: "${DENDRA_CAPACITY_URL:=$(sed -n 's/^DENDRA_CAPACITY_URL=//p' "$ENVF" | head -1 | tr -d '[:space:]"')}"
...
# THERE ARE TWO ROUTES, AND THE SECOND ONE NEEDS NOTHING FROM THIS HOST.
# Route 1 signs here: it needs the signer file AND a python3 on this machine. Route 2 signs INSIDE the
# miner container, which carries its own python3 and its own copy of the signer -- that is the whole
# point of asking it, since the keyring lives in its volume and the host cannot read it.
# The two used to share one `if` that required host python3 up front, so route 2 was gated behind a
# prerequisite it does not use. On the host this very file calls NOMINAL a few lines below -- "runs
# Docker and nothing else", therefore no python3 -- signing was structurally impossible, and the file
# said so about itself while requiring the opposite. A script is not a reliable source on its own
# effects: the condition is what runs, the comment is not.
#
# AND THE REPORT SEPARATES THREE FACTS, NOT TWO. "Signing failed" and "signing was never attempted"
# are different events, and the second one has a cause worth naming. The deposit is ACCEPTED either
# way (ADR-045 (10)), so this line is the only trace an operator ever gets: one that blames the
# identity for a missing python3 sends them to re-check the one thing that was right.
#
...
_SIGN_TRIED=""; _SIGN_WHY=""; _mc=""
if [ -r "$SIGNEUR" ] && command -v python3 >/dev/null 2>&1; then
...
# A miner installed the documented way runs in Docker, and its keyring lives in the container
# VOLUME: the host cannot read it, so signing here fails for a reason that has nothing to do with a
# wrong key. Falling back rather than replacing keeps a bare-metal install, where the host keyring
# is the real one, working.
if [ "${#SIGN_ARGS[@]}" -eq 0 ] && command -v docker >/dev/null 2>&1; then
  _mc="$(docker ps --filter 'name=miner' --format '{{.Names}}' 2>/dev/null | head -1)"
  if [ -n "$_mc" ]; then
    _SIGN_TRIED="${_SIGN_TRIED:+$_SIGN_TRIED, then }the container $_mc"
    while IFS= read -r _l; do SIGN_ARGS+=("$_l"); done < <(
...
if [ "${#SIGN_ARGS[@]}" -gt 0 ]; then
  ...
  say "  [!] UNSIGNED deposit, accepted but outside the verified block"
...
if not n: print("     [!] still 0. TWO causes produce this, and this line does not distinguish them:",
                "the report was posted UNSIGNED (see the signing line above), or the identity is absent",
                "from the on-chain registry. Read the signing line first, then:",
                "curl -s $DENDRA_REST/dendra/jobs/v1/miner")' 2>/dev/null || true
  else
