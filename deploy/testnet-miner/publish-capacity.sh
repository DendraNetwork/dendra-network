...
if [ -n "$SIGN_KEY" ] && [ -r "$SIGNEUR" ] && command -v python3 >/dev/null 2>&1; then
  ...
  # wrong key. The container holds both the keyring and dendrad, so ask the container. Falling back
  # rather than replacing keeps a bare-metal install, where the host keyring is the real one, working.
  if [ "${#SIGN_ARGS[@]}" -eq 0 ] && command -v docker >/dev/null 2>&1; then
    _mc="$(docker ps --filter 'name=miner' --format '{{.Names}}' 2>/dev/null | head -1)"
    if [ -n "$_mc" ]; then
      while IFS= read -r _l; do SIGN_ARGS+=("$_l"); done < <(
        printf '%s' "$JSON" | docker exec -i           -e DENDRA_SIGN_KEY="$SIGN_KEY" -e MINER_ID="$MINER_ID" -e DENDRA_NODE="$NODE"           -e DENDRA_KEYRING_DIR="${DENDRA_CONTAINER_KEYRING:-/data/keys/cosmos}"           -w /app "$_mc" python3 capacity_sign.py 2>/dev/null || true)
