...
while [ $# -gt 0 ]; do
  case "$1" in
    --fresh) FRESH=1 ;;
    -*)      echo "[join-val] FATAL: unknown option: $1"; echo "  usage: bash join_validator.sh <REMOTE_IP> [--fresh]"; exit 2 ;;
    *)       [ -z "$VPS" ] || { echo "[join-val] FATAL: two addresses given ($VPS, $1)"; exit 2; }
             VPS="$1" ;;
  esac
  shift
done
[ -n "$VPS" ] || { echo "[join-val] FATAL: missing <REMOTE_IP>"; echo "  usage: bash join_validator.sh <REMOTE_IP> [--fresh]"; exit 2; }
