...
# THREE ANSWERS, NEVER TWO — and this was the only one of the three that did not apply the rule.
# A tx rejected at block level exits 1 above ("The validator does NOT contribute to the seed");
# a confirmed one falls through to 0. Failing to CONFIRM within the window is neither: the tx may
# well be in a block already. Exiting 0 made "I could not look" indistinguishable from "I looked
# and it is fine", so the caller's `|| die` could never bite. Exiting 1 would be the opposite lie —
# announcing a failure nobody measured, which is exactly the false negative this file was already
# burned by (see the note above about an anchoring that WORKED reported as ANCHORING FAILED).
# Hence a DISTINCT code, and a caller that reads it.
NOT_CONFIRMED=0
if [ "$CONFIRMED" != 1 ]; then
  echo "[vrf] NOT CONFIRMED: the inclusion of $TXH could not be observed within 36 s."
  echo "      This is NOT a failure and NOT a success — the tx may already be in a block."
  echo "      Check by hand before counting this validator as contributing:"
  echo "        $D query tx $TXH --node $NODE"
  NOT_CONFIRMED=1
fi
...
# The status is rendered LAST, after the instructions above: they are needed whether or not the
# inclusion could be confirmed, and an early exit would have swallowed them.
[ "$NOT_CONFIRMED" = 1 ] && exit 2
exit 0
