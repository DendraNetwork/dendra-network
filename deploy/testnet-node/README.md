...
- The operator's `network-info.txt` carries **the same value** in `SEEDS` and in `PERSISTENT_PEERS`:
  `publish_network.sh` assigns one from the other, so the two fields can never disagree. How many
  hosts that value names is a **reading, not a property** - open the file and count. If it names one,
  that is two fields, one machine, one point of failure.
...
- Nodes behind home NAT make this worse: with `external_address = ""` the node advertises itself as
  ...
  redundancy option for anyone else. If you can forward port **26656**, set **`DENDRA_EXTERNAL_ADDRESS=<your public host:port>`** in
  `.env` and restart. The node passes it to `dendrad` as `--p2p.external-address`, so removing the
  variable reverts cleanly — nothing is written into the volume. It **refuses to start** on a
  wildcard, loopback or RFC1918 value rather than gossiping one: peers store what you advertise and
  re-gossip it, so a wrong address is worse than none. And it only helps once the port is genuinely
