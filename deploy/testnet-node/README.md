...
```
Local RPC at `http://localhost:26657/status` (or `DENDRA_RPC_PORT` if you changed it).
...
a **validator** that falls behind gets jailed and slashed. The numbers below were measured on this
testnet on 2026-08-07 — re-measure them before trusting them, the chain moves.

**What the chain demands.** `dendrad query slashing params` on this network returns the Cosmos SDK
defaults: `signed_blocks_window=100`, `min_signed_per_window=0.500000000000000000`,
`downtime_jail_duration=10m0s`, `slash_fraction_downtime=0.010000000000000000` (and
`slash_fraction_double_sign=0.050000000000000000`, which no restart will ever forgive — that one
tombstones you). So you are jailed and lose **1% of your stake** as soon as you miss
**51 blocks inside a 100-block window**. The block interval measured over 850 blocks
(heights 91034→91884, chain timestamps 2026-08-06T23:44:45Z → 2026-08-07T00:56:01Z, 4275.535 s) is
**5.0300 s per block** — that interval is *not* fixed by the consensus, it emerges from
`timeout_commit = "5s"` and propagation, so treat it as an observation, not a constant. At that
observed interval, **51 missed blocks is about 4 min 17 s of downtime**.

Four minutes. Not four hours. This is the whole point:
...
- **A validator is not a desktop.** A machine that sleeps, reboots for updates, or gets its Docker
  ...
  observed interval**, against a 51-block budget. In the same 3-hour log window the process caught
...
- The reason is structural: at the time of measurement the public seed node reported `n_peers=1`, and
  ...
  same host, and any hiccup on it put every validator on the clock toward the 51-block threshold.
...
**What to watch** (every minute is reasonable; alert well before the 4-minute budget):
...
> counts against the 51-block budget: unjail *after* the restart, not before.
>
