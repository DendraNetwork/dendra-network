  ...
  if curl -fsSL "$GENESIS_URL" -o "$tmp" 2>/dev/null; then
    ...
    if [ "$sha" = "$GENESIS_SHA256" ]; then
      ...
      if [ "$pin_ok" = 1 ] && [ "$pin_sha" = "$sha" ]; then
        ...
        say "       Update:  git fetch origin && git reset --hard origin/main   then re-run,"
        say "       or set DENDRA_ALLOW_UNVERIFIED_GENESIS=1 for a network you already trust."
        ...
        say "       Most likely: your clone predates a network relaunch -> git fetch origin && git reset --hard origin/main, then re-run."
...
EOF
  ...
  # to what happened.
  if wait_healthy "$MINER_KIT" miner; then
    print_status_miner
  ...
  # now needs. What changes is that the status stops claiming a success nobody measured.
  print_status_miner
...
wait_for_sync(){
  ...
  while :; do
    ...
    # is merely behind pays one status read and one block read per report interval it rests on, which
    # is nothing, while a node that disagrees is told so in two minutes instead of fifteen. And once
