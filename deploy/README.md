...
|------|-------------|---------|---------|
| **Validator** | A small **genuinely** always-on server + stake. **No GPU.** ⚠️ Downtime is *slashed*, not merely unproductive — [read the cost first](#-the-role-costs-money-when-your-machine-goes-quiet--read-this-before-you-bond) | Produce blocks, and **contribute to the VRF beacon** — see the note below. ⚠️ **Which role the network is short of is a STATE, not a property of the role**: read it from the two counters below (`latest_contributors` against `committee_min_vrf_contributors`, and the registered-miner total against `audit_min_quorum + 1`) rather than from this table, which cannot follow a bond | `bash deploy/join.sh --validator` |
...
>
> That checkpoint is gated by **two** independent floors. Clearing the first does not clear the
> second, and only the second gates pay. **Which side of either one this network sits on is a
> reading, not a property of this page** - the commands below return it.
>
...
>
> ```bash
> curl -s http://api.dendranetwork.com:26657/status | python3 -c "import json,sys; print(json.load(sys.stdin)['result']['sync_info']['latest_block_height'])"   # target
...
| `deploy/install.sh` · `deploy/install.ps1` | **You**, on a bare machine | Prepare the host (Docker, Compose v2, NVIDIA toolkit, the clone; on Windows WSL 2 first), then run `join.sh`. Plan first, nothing without `--yes`. |
...
| `services/` — relay, capacity server, faucet, gateway | **The network operator** | Runs on the operator's host; you reach it over HTTP. |
...
- your GPU reaches **tier ≥ 3** *and* the model it sizes to is on the judge allow-list (`mistral-nemo`,
  `qwen3:30b-a3b-instruct-2507-q4_K_M`). ⛔ **Reaching the tier is not enough, and the difference is
  not intent — it is a draw.** The probe picks ONE family out of that tier's candidates, hashed on
  your machine, so a tier whose candidate list is wider than the allow-list seats a judge only when
  the draw lands on an allow-listed family. Tier 3 offers three candidates against two allow-listed
  names; tier 5 offers only the allow-listed one. The pick is stable across restarts, so it is not
  something you retry — read it with the command below before buying anything.
  `DENDRA_JUDGE_ALLOWLIST` does **not** widen this: it is INTERSECTED with the project's list, so it
  can only forbid a validated model on your own box, never seat one the project has not validated;
  **or**
...
curl -s http://api.dendranetwork.com:26657/status | python3 -c "import json,sys; s=json.load(sys.stdin)['result']['sync_info']; print(s['latest_block_height'], s['latest_block_time'])"
```
