...
step "0b) GATE: the binary about to be built must correspond to a COMMIT"
...
_IMG_DIRTY="$(git -C "$REPO" status --porcelain -- chain docker/Dockerfile.chain docker/entrypoint-chain.sh 2>/dev/null)" || _IMG_DIRTY="?"
if [ "$_IMG_DIRTY" = "?" ]; then
  ...
  if [ "${DENDRA_ALLOW_DIRTY_BUILD:-0}" = "1" ]; then
    ...
    die "cannot verify that the chain image sources are committed -> refusing to deploy blind."
  fi
elif [ -n "$_IMG_DIRTY" ]; then
  echo "  the following paths ENTER the chain image and are not committed:"
  printf '%s
' "$_IMG_DIRTY" | head -10 | sed 's/^/    /'
  if [ "${DENDRA_ALLOW_DIRTY_BUILD:-0}" = "1" ]; then
    ...
    die "chain image paths are uncommitted -> refusing to deploy an unreproducible binary."
  fi
fi
echo "  OK the chain image is built from committed sources"
...
if [ "${DENDRA_FRESH:-0}" = "1" ]; then
  ...
  _VOL="$($RSH 'docker volume inspect dendra_dendra-home >/dev/null 2>&1 && echo PRESENT || echo ABSENT' 2>/dev/null || echo UNREADABLE)"
  case "$_VOL" in
    ...
    PRESENT)
      ...
      _WHY="$($RSH 'docker volume rm dendra_dendra-home 2>&1; docker ps -a --filter volume=dendra_dendra-home --format "{{.Names}}"' 2>&1 || true)"
      die "3b: the chain volume dendra_dendra-home SURVIVED the purge. Booting now would replay the old
...
$(printf '%s' "$_WHY" | sed 's/^/       /')
     A volume that will not go is held by a container. Remove the holders FIRST, then the volume:
       ...
       ssh root@$VPS 'docker volume rm dendra_dendra-home'
...
# name, over HTTPS.
echo "  RPC http://$PUBHOST:26657 | gateway https://<certified-hostname>/v1 (Caddy) | faucet :4500 | chat :8080"
