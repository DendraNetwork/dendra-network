...
# TWO WAYS OUT OF THIS GUARD, BOTH MEASURED. (a) `export FOO=...`: the pattern anchored only
# `^[[:space:]]*IDENT=`, so any line prefixed with `export` -- which `set -a ; . file` treats exactly
# the same way -- went uncounted. (b) TAB indentation: `tr -d ' ='` strips the SPACE only, so the key
# stayed `<TAB>FOO` and never paired with `FOO` for `uniq -d`. Either way the guard reported GREEN on a
# file that assigns twice. So `export` is now consumed by the pattern, and the cleanup removes ALL
# whitespace (`[:space:]`), not just spaces: normalise what the SHELL normalises, not what the eye sees.
DUPES=$(grep -oE '^[[:space:]]*(export[[:space:]]+)?[A-Za-z_][A-Za-z0-9_]*=' "$ENVF" \
  | sed -E 's/^[[:space:]]*(export[[:space:]]+)?//; s/=$//' | sort | uniq -d)
if [ -n "$DUPES" ]; then
...
# A NON-ZERO EXIT CODE IS NOT A PROOF THAT THE GUARD BIT.
# These three checks used to accept any rc except 0 and 124. A service that dies of a syntax error, a
# missing import, a busy port or a bad interpreter exits non-zero too -- so a guard DELETED OUTRIGHT
# would still be reported OK, which is the exact inversion of what this section exists to show.
# Each refusal path exits with the documented code 2 AND prints `REFUSED:` with its reason, so the
# proof asks for both. Anything else is reported as INCONCLUSIVE rather than as a pass: a check that
# cannot tell a refusal from a crash must say so, not pick the reassuring reading.
refus_prouve() { # $1 = label, $2 = rc, $3 = output
  # ⛔ THE WORD, NOT THE WORDING. This demanded the literal string REFUSED, while the shipped faucet
  # says "FATAL: ... Refusing to boot." -- the guard bit, for the right reason, exit code 2 and all,
  # and this check reported INCONCLUSIVE and turned the whole gate RED. It blocked a public relaunch
  # with the chain already stopped. A checker that only recognises its own vocabulary does not measure
  # the guard, it measures itself. The DISCIPLINE is unchanged and is the part that matters: rc=2 alone
  # is never enough (a crash also exits non-zero), an explicit refusal must be stated -- but "REFUSED",
  # "refusing", "refus" are the same statement.
  if [ "$2" = "2" ] && printf '%s' "$3" | grep -qiE 'REFUS'; then
    echo "   OK  $1 (rc=2, refusal message present)"
  elif [ "$2" = "0" ]; then
    echo "   FAIL  $1: it BOOTED"; printf '%s
' "$3" | tail -2; FAIL=1
  else
    echo "   INCONCLUSIVE  $1: died with rc=$2 and no refusal message -- this proves nothing about the guard"
    printf '%s
' "$3" | tail -2; FAIL=1
  fi
}
