...
# The correct shape was already written in the development repository
# (`dendra_b02_slash_live.sh`), which is not part of a published tree.
...
# AN EMPTY SET USED TO PRODUCE THE STRONGEST CLAIM THIS SCRIPT CAN MAKE. `HONEST_INTACT` starts
# at 1 and is only ever lowered INSIDE the loop below; `HONEST_UNKNOWN` is only ever raised
# there too. With no judge to iterate over, both keep their initial values, `slash_proven` came
# out true and the banner printed "honest judges INTACT" — having measured nothing at all. The
# query above swallows its stderr, so an unreachable node reaches this line with an empty set
# and the artefact still claimed the proof. Zero judges is not "no false slash": it is no
# measurement, and the two must never render the same verdict.
# The guard COUNTS THE SET THE WAY THE LOOP WILL READ IT. `[ -n "$JUDGES" ]` was not enough:
# a whitespace-only value is non-empty by that test and still yields ZERO iterations below, which
# is the very hole being closed. Guard and consumer must split the value identically, or they can
# disagree — and it is the disagreement, not the emptiness, that produces a vacuous verdict.
_nj=0; for _j in $JUDGES; do _nj=$((_nj+1)); done
[ "$_nj" -gt 0 ] || die "no honest judge in the miner registry (query failed, or no juge<N> registered): intactness cannot be measured, so this script refuses to claim it."
