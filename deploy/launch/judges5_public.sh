...
if [ "$CAN_JUDGE" != "true" ]; then
  ...
  warn "honest miners). Seat the audit committee on a machine with >= 12 GB VRAM, or >= 26 GB RAM for the MoE judge."
...
for k in $(seq 1 "$JUDGES"); do
  ...
  # Count seats ACTUALLY FILLED, not seats requested. Reporting `$JUDGES` counts a judge with no address
  # (skipped above) or below the required stake (so it will never register) as if it were seated. Since
  # the audit floor is 4 seats, announcing 5 when 3 are real makes the slash look armed when it cannot
  # fire.
...
# The number of seats FILLED decides whether a slash can fire at all: below the quorum floor the audit is
# INERT and a cheater walks. Saying so here beats discovering it by staring at verdicts that never arrive.
if [ "$SEATED" -lt 4 ] 2>/dev/null; then
  say "  WARNING: $SEATED seats filled < quorum floor (4) -> NO hard slash can fire (the audit is inert)."
