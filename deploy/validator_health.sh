...
if [ "$CHAIN_STALLED" = 1 ]; then
  _problem "THE CHAIN IS NOT PRODUCING BLOCKS. The last block is $CHAIN_AGE s old, past the bound above. Every other number in this report is read FROM the chain, so on a stopped chain they all stay green: 'not catching up' means there is nothing to catch up with, and the missed-block counter freezes because missed blocks are only counted on blocks that happen. Look at the OTHER validators, not at this one -- with the voting power split so that no single validator reaches two thirds, one node being down stops the chain for everyone, and it restarts when that node comes back."
