...
if [ "$SEED_OK" = 1 ]; then
  ...
  SEED_PW="$(_seedint "$TMP/seed.json" contributor_power_bps)"
  case "$SEED_PW" in ''|*[!0-9]*) SEED_PW="?";; esac
  ...
  if [ "$SEED_PW" = "?" ]; then
    printf '  seed power      : contributor_power_bps=? -- ABSENT from the answer. The field IS part of\n'
    printf '                    committee-seed-health, so a node that omits it is running a binary from\n'
    printf '                    before it was added: upgrade it, or read the second bar from the node log\n'
    printf '                    line "decentralized VRF seed" in the meantime.\n'
    printf '                    Until then this is a GAP, not a pass: a contributor COUNT sitting at its\n'
    printf '                    floor does NOT prove the seed is decentralised -- the power bar can still\n'
    printf '                    refuse it, and the draw falls back to the legacy source in silence.\n'
  elif [ "$SEED_PW" -lt 6667 ] 2>/dev/null; then
    printf '  seed power      : contributor_power_bps=%s < 6667 -> UNDER THE SECOND BAR: LEGACY fallback,\n' "$SEED_PW"
    printf '                    anti-grinding INACTIVE even though the contributor COUNT is met.\n'
  else
    printf '  seed power      : contributor_power_bps=%s (>= 6667, second bar cleared)\n' "$SEED_PW"
  fi
