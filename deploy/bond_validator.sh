...
confirm_execution() {
  ...
  # A BROADCAST THE MEMPOOL REFUSED IS NOT A SLOW ONE, AND POLLING FOR IT ANSWERS THE WRONG QUESTION.
  # A non-zero code on the BROADCAST answer is the ante-handler's verdict: the transaction was never
  # queued, so no block can ever carry it. Judged on the hash alone -- and a rejected broadcast HAS a
  # hash -- this case ran the twelve polls below and then printed "never made it into a block": the
  # exact same sentence as a transaction that WAS accepted and simply came late. Two opposite facts
  # rendered indistinguishable, sixty seconds spent querying a hash that cannot exist, and the reason
  # discarded with $_BR, which nothing else ever prints. The reader is the one this file already
  # applies to the funding transfer, not a second rule written here.
  printf '%s' "$_BR" | broadcast_accepted || {
    # SAY WHAT WAS MEASURED, NOT WHAT IT USUALLY MEANS. The broadcast returned a non-zero code:
    # that is the fact. The usual cause is an ante-handler refusal (funds, sequence, gas), but a
    # re-broadcast of identical bytes also answers non-zero ("tx already exists in cache") while
    # the transaction IS queued -- so claiming "never queued" would state a cause we did not read.
    say "  [ERR] $_LBL was REFUSED AT BROADCAST by the node (non-zero code), so it did not enter the mempool as a new tx."
    say "        Usually an ante-handler refusal (funds, sequence, gas); a re-broadcast of the same tx answers this way too."
    say "        The node's own answer: $(printf '%s' "$_BR" | tail -3)"
    return 1
  }
  _CODE=""
  ...
  # THREE ANSWERS, NEVER TWO — the same rule this file states thirty lines below for `catching_up`,
  # and it was broken here. After the poll window the old text said "tx never made it into a block"
  # and the caller turned that into "the bond is NOT effective". Both are ASSERTIONS ABOUT THE CHAIN
  # that nothing measured: the only fact established is that no execution result could be READ in
  # time. A slow block, a lagging local node or a busy mempool produces exactly this, on a bond that
  # went through. The cost of getting it wrong is not a wrong line on a screen: the operator reads
  # "NOT effective" and re-runs, and a re-run is a SECOND create-validator transaction.
  # Stopping is right; claiming a cause is not. Hence code 2 — not measurable — distinct from the 1
  # returned by the two refusals this function actually READS (refused at broadcast, non-zero code).
  [ -n "$_CODE" ] || {
    say "  [!] $_LBL: could NOT CONFIRM the execution of tx $_HASH within the poll window."
    say "      This is an ABSENCE OF MEASUREMENT, not a verdict: the transaction may well be in a block."
    say "      Read it by hand before anything else — a re-run is a SECOND transaction:"
    say "        $DC exec -T node dendrad query tx $_HASH --node $NODE_LOCAL -o json"
    return 2
  }
...
if [ "$ALREADY" = 1 ]; then
  ...
  # The two refusals are not the same fact, so they do not get the same sentence. Both stop the
  # script — an unconfirmed bond is not a green light either — but only one of them is a verdict.
  confirm_execution "$BR" "create-validator" || case $? in
    2) die "the bond is NEITHER proven effective NOR proven failed — the execution could not be read in time. Check the tx printed above by hand; do NOT re-run blind, that would broadcast a second one." ;;
    *) die "the bond is NOT effective — nothing else in this script may proceed" ;;
  esac
...
tr -d '\r' < "$REPO/deploy/testnet/anchor_vrf_key.sh" | $DC exec -T \
  ...
  node bash -s
_VRF_RC=$?
# THREE ANSWERS FROM THAT SCRIPT, AND THEY MUST NOT COLLAPSE HERE EITHER. A bare `|| die` turns
# "could not confirm" into "anchoring failed" — a claim about a measurement nobody made. Before
# the fix on the other side it could not even bite, since the unconfirmed case exited 0.
case "$_VRF_RC" in
  0) ;;
  2) VRF_UNCONFIRMED=1
     say "  [!] VRF anchoring NOT CONFIRMED within the window. This is NOT a failure and NOT a"
     say "      success: the transaction may already be in a block. Verify it by hand before"
     say "      counting this validator as contributing to the committee seed." ;;
  *) die "VRF anchoring failed — see the output above" ;;
esac
...
say ""
[ "${VRF_UNCONFIRMED:-0}" = 1 ] && say "  [!] REMINDER: the VRF anchoring above was NOT CONFIRMED — treat the panel as unproven until you have checked the tx by hand."
