...
if [ ! -f "$GEN" ]; then
  ...
  # THE RESERVE GOES TO THE MODULE ACCOUNT, AND THAT ADDRESS IS DERIVED, NEVER NAMED BY A KEY.
  # Taking it from a keyring key called "reserve" funds an ORDINARY account: the coins land at an
  # address the `emission` module cannot spend, while its `Reserve` counter still announces them. The
  # chain then boots, looks healthy, and defers every epoch from the first one with
  # `security_transfer_failed` -- reward budget unreachable, no message before that block. That is the
  # state the live chain is in, and no transaction repairs it: it is decided here or not at all.
  # The derivation is `authtypes.NewModuleAddress("emission")` = bech32(sha256("emission")[:20]),
  # computed rather than copied. The constant below is a CROSS-CHECK, not the source: if the prefix or
  # the derivation ever changes, the boot stops instead of quietly funding a different address.
  RES=$(python3 - <<'PYEOF'
import hashlib
...
def cb(d):
    ...
    for x in d:
        a=(a<<8)|x; b+=8
        while b>=5: b-=5; o.append((a>>b)&31)
    if b: o.append((a<<(5-b))&31)
    return o
raw=hashlib.sha256(b"emission").digest()[:20]; d=cb(list(raw))
c=pm(hx("dendra")+d+[0]*6)^1
print("dendra1"+"".join(CH[x] for x in d)+"".join(CH[(c>>5*(5-i))&31] for i in range(6)))
PYEOF
)
  RES_ATTENDU="dendra1q6tym5h6cd3cxcqua9eflkf5rct8c6rsn92wrf"
  [ "$RES" = "$RES_ATTENDU" ] || {
    echo "[chain] FATAL: the derived emission module address ($RES) does not match the pinned one ($RES_ATTENDU)."
    echo "        Either the bech32 prefix or the module derivation changed. Funding a different address"
    echo "        would strand the whole Reserve, so the boot stops here."; exit 1; }
  if [ -n "${DENDRA_RESERVE_ADDR:-}" ] && [ "$DENDRA_RESERVE_ADDR" != "$RES" ]; then
    echo "[chain] FATAL: DENDRA_RESERVE_ADDR=$DENDRA_RESERVE_ADDR is not the emission module account ($RES)."
    echo "        The Reserve is not an ordinary holding: x/emission spends it BY MODULE NAME, so any"
    echo "        other address makes the counter unbacked and every epoch defer."; exit 1
  fi
  ...
  echo "[chain] reserve = EMISSION MODULE ACCOUNT $RES (derived, no key holds it)"
  ...
  # `add-genesis-account` creates a BaseAccount. The balance alone would work today -- bank resolves
  # the module by name to this same address -- but auth would hold an ordinary account where a module
  # lives, which is the "works by accident" shape. It is promoted to a real ModuleAccount so the state
  # says what it is. Empty permissions: emission neither mints nor burns (app_config.go).
  _g="$HOME_DIR/config/genesis.json"; _t=$(mktemp)
  jq --arg a "$RES" '
    .app_state.auth.accounts |= map(
      if .address == $a then
        {"@type":"/cosmos.auth.v1beta1.ModuleAccount",
         "base_account":{"address":$a,"pub_key":null,"account_number":.account_number,"sequence":"0"},
         "name":"emission","permissions":[]}
      else . end)
  ' "$_g" > "$_t" && mv "$_t" "$_g"

  # EXECUTED, on a throwaway home, in the image that runs this file (`dendra/chain:latest`, jq 1.6).
  # The dry run earned its keep: the two jq programs below had shipped WRAPPED IN LITERAL QUOTES, so
  # jq received a broken filter, `$a` was never interpolated, and the check compared the filter TEXT
  # to the expected amount. `bash -n` cannot see that -- a mangled quote is valid shell. The corrected
  # run boots the chain and reports 3300000000000 udndr at the emission ModuleAccount.
  #
  # THE ARTEFACT IS CONFRONTED TO THE REQUIREMENT, NOT TO THE INTENTION.
  # The source has always declared this pocket funded; the deployed genesis still credited an ordinary
  # account. A gate reading the declaration would have passed, so this one reads the BUILT FILE.
  _bal=$(jq -r --arg a "$RES" '.app_state.bank.balances[] | select(.address==$a) | .coins[] | select(.denom=="udndr") | .amount' "$_g")
  _typ=$(jq -r --arg a "$RES" '.app_state.auth.accounts[] | select((.base_account.address // .address)==$a) | ."@type"' "$_g")
  _nom=$(jq -r --arg a "$RES" '.app_state.auth.accounts[] | select((.base_account.address // .address)==$a) | .name // ""' "$_g")
  [ "$_bal" = "3300000000000" ] || { echo "[chain] FATAL: genesis balance at the emission module is '$_bal', expected 3300000000000."; exit 1; }
  [ "$_typ" = "/cosmos.auth.v1beta1.ModuleAccount" ] || { echo "[chain] FATAL: the emission pocket is a '$_typ', not a ModuleAccount."; exit 1; }
  [ "$_nom" = "emission" ] || { echo "[chain] FATAL: the module account at $RES is named '$_nom', not emission."; exit 1; }
  echo "[chain] VERIFIED in the built genesis: $_bal udndr at the emission ModuleAccount $RES"
  ...
  # but has no bite. The anchor INTENDED by ADR-033 is `min_stake`.
  # ⛔ THAT EQUALITY IS BROKEN TODAY AND THIS COMMENT USED TO HIDE IT. It still read "the anchor
  # chosen is min_stake (50000 udndr = 0.05 DNDR)" while the same file sets min_stake to 1000000
  # a hundred lines below, and the live chain applies 1000000 against a dispute_bond of 50000 --
  # a ratio of 1:20, measured, not deduced. Disputing therefore costs a twentieth of a miner
  # identity: the same capital that buys one identity buys twenty unfounded disputes.
  # THE VALUE IS AN ECONOMIC PARAMETER AND IT IS THE OWNER'S CALL, so nothing is changed here --
  # but the justification no longer claims an equality that does not hold. About 11 job prices
  ...
  # Lowering it is NOT a silent downgrade: in optimistic mode `Params.Validate()` rejects a zero
  # `committee_reveal_delay` outright -- a decentralised seed does not buy an exemption -- and it also
  # rejects a non-zero `committee_min_vrf_contributors` posted without the source that reads it.
  # (Cited by NAME, never by line number: the two it used to cite had both drifted.)
  # The refusal lands on `gentx` below, which re-validates the
  ...
  if [ "$RVDELAY" = "0" ]; then
    echo "[chain] committee_reveal_delay=0 -> the job beacon is frozen INSIDE the opening block."
    echo "        In optimistic mode (verification_mode=1) the chain REFUSES this at load: the boot"
    echo "        will fail on Params.Validate, it will not start a weaker chain. Setting"
    echo "        committee_seed_source=1 does NOT lift the refusal, and that is the point -- a"
    echo "        decentralised source stops a single actor from PICKING the seed, not the PROPOSER"
    echo "        from seeing it. The proposer composes the very block the seed is fixed in: it reads"
    echo "        the pending job, assembles the vote extensions the seed is built from, and knows the"
    echo "        committee before it commits to proposing."
    echo "        In redundant mode (verification_mode=0) a zero delay stays a valid choice -- peer"
    echo "        divergence carries the verification there, so the committee is not the only check."
  fi
  ...
  # ⛔ juror_freshness_blocks WAS WRITTEN BY NOBODY, AND THAT SILENTLY DISCARDED A GOVERNANCE VOTE.
  # On the chain destroyed on 2026-08-13 it was the ONLY one of the 36 module parameters that ever
  # changed: proposal 2 carried it from 0 to 49500, and it passed. Neither this patch nor
  # `DefaultParams()` assigns it, so a relaunch restarts it at 0 — which falls back to the compiled
  # constant `jurorFreshnessBlocks`, i.e. a window five times wider than the one that was voted.
  # Nothing on screen would have said so: a parameter absent from a patch reads exactly like a
  # parameter deliberately left at its default.
  # ⚠️ It belongs in THIS patch, the one whose failure is FATAL (see below), and not in the second
  # one where a failure is only a WARNING: a jury window is a security bound, and a boot that
  # silently loses it is worse than a boot that refuses.
  # The value is in BLOCKS — the unit the chain counts. Converting it to days would require an
  # inter-block interval that the consensus does not fix (it emerged at ~5.24 s on the previous
  # chain, against the 1 s that the code comments long assumed).
  JFB="${DENDRA_JUROR_FRESHNESS_BLOCKS:-49500}"
  case "$JFB" in ''|*[!0-9]*) echo "[FATAL] DENDRA_JUROR_FRESHNESS_BLOCKS=$JFB is not a block count"; exit 1;; esac

  # ⛔ AT 50000 udndr (0.05 DNDR) AN IDENTITY COST NOTHING, AND TWELVE OF THEM SAT ON EVERY JURY.
  # Measured on the chain destroyed on 2026-08-13: thirteen miners registered, twelve had never
  # anchored anything, and the whole set cost about 0.65 DNDR — less than a tenth of ONE faucet grant
  # (10 DNDR). They were not attackers; they were free, and free identities fill the seats that decide
  # a slash. `antievasion.go` already warns that four identities at `min_stake` could take 80 % of an
  # honest bond for 0.2 DNDR: the entry price was the assumption under that warning, and it was too
  # low for the warning to hold.
  #
  # 1 000 000 udndr = 1 DNDR makes thirteen identities cost 13 DNDR, i.e. MORE THAN ONE GRANT, so a
  # sybil set becomes a day of faucet requests instead of one click. It stays cheap for a real miner:
  # the assignment ceiling saturates the draw weight at twice this floor (ADR-042), so 2 DNDR out of
  # the 10 a joiner receives buys the maximum the draw can give, and the rest stays spendable.
  # ⚠️ This is the SYBIL price, NOT the anti-monopoly lever. Concentration is bounded in the code now,
  # and no genesis value could do that job: `params.proto` has no stake ceiling to set.
  MINSTAKE="${DENDRA_MIN_STAKE:-1000000}"
  case "$MINSTAKE" in ''|*[!0-9]*) echo "[FATAL] DENDRA_MIN_STAKE=$MINSTAKE is not a udndr amount"; exit 1;; esac
  [ "$MINSTAKE" -gt 0 ] || { echo "[FATAL] DENDRA_MIN_STAKE=0 disables the assignment ceiling (ADR-042): it is scaled by min_stake"; exit 1; }
  # SEEDED MODELS ARE INACTIVE UNLESS THEY CARRY A DIGEST, AND THAT IS THE POINT.
  #
  # `weights_sha256` is what binds a `model_id` declared in a commit to a real artifact. This kit used
  # to write three models ACTIVE with an EMPTY digest into every genesis it built. The chain says so at
  # InitChain -- "ACTIVE model(s) at genesis WITHOUT weights_sha256 ... Fix BEFORE any incentivized
  # launch" -- and deliberately does NOT refuse: a development genesis legitimately carries
  # placeholders, and refusing would replace a missing guard with a chain that will not boot. So the
  # alert was the only mechanism, and an alert in a boot log is read once and never again.
  #
  # The guard belongs HERE, where the genesis is composed. `active` is consulted at exactly one site
  # (`x/jobs/keeper/msg_server_commit.go`) and only under `enforce_model_registry`, which is false
  # in this genesis -- so seeding these models INACTIVE changes nothing today. It changes one thing the
  # day the registry is armed: the only way to activate a model becomes `RegisterModel`, which REFUSES
  # an empty digest (`msg_server_model.go:33`). The precondition stops being a sentence in a log and
  # becomes the shape of the path.
  #
  # The escape hatch is explicit rather than implicit, because someone will want the old behaviour and
  # should have to say so: DENDRA_GENESIS_MODELS_ACTIVE=1 restores `active:true`, and the boot then
  # prints what that costs.
  MODELS_ACTIVE="${DENDRA_GENESIS_MODELS_ACTIVE:-0}"
  case "$MODELS_ACTIVE" in
    0|1) ;;
    *) echo "[chain] FATAL: DENDRA_GENESIS_MODELS_ACTIVE must be 0 or 1. Got: $MODELS_ACTIVE"; exit 1 ;;
  esac
  if [ "$MODELS_ACTIVE" = "1" ]; then
    _MACT=true
    echo "[chain] WARNING: seeded models are ACTIVE with an EMPTY weights_sha256"
    echo "        (DENDRA_GENESIS_MODELS_ACTIVE=1). There is then no model_id<->artifact binding: a"
    echo "        miner can declare one of these models and serve something else. Acceptable on a"
    echo "        devnet, NOT before an incentivized launch."
  else
    _MACT=false
    echo "[chain] seeded models are INACTIVE (no weights_sha256 to bind them to an artifact)."
    echo "        Nothing changes while enforce_model_registry is false. To activate one, register it"
    echo "        with its digest (tx modelregistry register-model), which is the path that requires"
    echo "        the binding. DENDRA_GENESIS_MODELS_ACTIVE=1 restores the old behaviour."
  fi
  jq --arg asb "$ASB" --arg arto "$ARTO" --arg dbond "$DBOND" --arg seed "$SEEDSRC" --arg rvd "$RVDELAY" \
     --argjson mact "$_MACT" \
     --arg jfb "$JFB" --arg minstake "$MINSTAKE" '
    del(.app_state.mint)
    | .app_state.jobs.params.juror_freshness_blocks = $jfb
    | .app_state.jobs.params.verification_mode = "1"
    | .app_state.jobs.params.audit_sample_bps = $asb
    | .app_state.jobs.params.min_stake = $minstake
    ...
    | .app_state.modelregistry.models = [
        {"id":"llama3.1:8b-instruct-q4_K_M","weights_sha256":"","quant":"q4_K_M","engine":"ollama","hw_class":"consumer-gpu","active":$mact,"hf_repo":"","hf_revision":""},
        {"id":"mistral-nemo","weights_sha256":"","quant":"","engine":"ollama","hw_class":"consumer-gpu","active":$mact,"hf_repo":"","hf_revision":""},
        {"id":"nomic-embed-text","weights_sha256":"","quant":"","engine":"ollama","hw_class":"consumer-gpu","active":$mact,"hf_repo":"","hf_revision":""}
      ]
  ...
  # ⛔ THE TWO TESTS BELOW ARE NOT AN INTEGER TEST, THOUGH BOTH MESSAGES SAY "integer".
  # `[ " 2" -ge 1 ]` and `[ "+2" -ge 1 ]` are TRUE in bash -- the numeric comparison skips blanks and
  # accepts a leading sign -- so " 2", "2 " and "+2" walk past the seed_source=1 branch, and " 0 ",
  # "+0" and "-0" walk past the seed_source=0 one. The `2>/dev/null` plus `!` catches a WORD; it was
  # never asked to catch a number spelled differently. What follows then writes
  # committee_min_vrf_contributors=" 2" into the genesis, where params.proto declares a uint64, and Go
  # refuses every one of those spellings -- so the boot dies further down in a codec trace, which is
  # the exact outcome this guard says above it exists to replace. Measured: no accepted spelling
  # parses to 0, so the floor itself still refuses a one-validator seed; what was lost is the readable
  # refusal, not the bar.
  # The same file already refuses this class twice, for DENDRA_JUROR_FRESHNESS_BLOCKS and
  # DENDRA_MIN_STAKE, in the form reused here: two sibling guards in one script must not disagree
  # about what an integer is. Plain digits only, which is what the chain reads back.
  case "$MINVRF" in
    ''|*[!0-9]*) echo "[chain] FATAL: DENDRA_MIN_VRF_CONTRIB must be a non-negative integer written in plain digits -- no sign, no surrounding spaces, no 0x/1e form. It is written into the genesis as a uint64 (params.proto) and any other spelling makes the genesis unloadable at gentx, far from here. Got: [$MINVRF]"; exit 1 ;;
  esac
  if [ "$SEEDSRC" = "1" ]; then
  ...
  # The values a chain inherits when nobody decides anything are the pinned SDK's:
  # signed_blocks_window=100, min_signed_per_window=0.5, downtime_jail_duration=600s,
  # slash_fraction_downtime=0.01 (x/slashing/types/params.go:12-19). Every chain booted from this file
  # before the section below existed ran exactly those, and the network running today moved off them by
  # governance rather than by genesis -- so the live values are a QUERY, never a line in a comment.
  #
  # WHAT THEY DECIDE, READ FROM THE FUNCTION THAT APPLIES THEM RATHER THAN FROM THEIR NAMES.
  # `HandleValidatorSignature` computes, in the cosmos-sdk (x/slashing/keeper/infractions.go --
  # a dependency, not this tree):
  ...
  #    the SDK defaults allow (~4 min 17 at the same 5.03 s/block) -- 29x. A validator holding
  ...
  # and subtracts it in cosmos-sdk's x/slashing/keeper/infractions.go::HandleValidatorSignature.
...
if command -v dendra-vrf >/dev/null 2>&1; then
  ...
  (
    ...
    # THE SECRET REACHES THE CLI BY THE ENVIRONMENT, NEVER BY argv. `ps` and /proc/<pid>/cmdline
    # are world-readable, so a key on the command line is readable by any local process for as long
    # as the command runs; /proc/<pid>/environ is owner-only. `dendra-vrf` has honoured
    # DENDRA_VRF_SK from the start and says so in its own usage — the safe channel was BUILT, and
    # this caller was the one place still ignoring it. `miner.py` was fixed and the rule was
    # never carried here.
    # This key is the VALIDATOR's, the one `loadNodeVrfKey()` reads for vote extensions at every
    # height. Stolen alone it forges nothing — an accepted contribution also needs the consensus
    # key — but it makes this validator's contribution PREDICTABLE, and unpredictability is the
    # whole property the key exists to provide.
    # VSK stays a shell variable and is never exported: only the two children below receive it.
    VSK=$(cat "$VRFKEY" 2>/dev/null)
    VPK=$(DENDRA_VRF_SK="$VSK" dendra-vrf pubkey 2>/dev/null)
    A=$($BIN keys show validator -a $KB 2>/dev/null)
    POP=$(DENDRA_VRF_SK="$VSK" dendra-vrf prove "dendra/vrf-pop/$A" 2>/dev/null)
...
# ZERO RULE: a chain answer is PARSED, never grepped. proto3 OMITS a zero, so on a SUCCESSFUL
# transaction `code` is ABSENT and a textual predicate reads that as "never included" -- it then
# re-broadcasts an anchoring that already worked and reports it as NOT confirmed. `txhash` proves an
# answer arrived; once it is there, an absent `code` IS a zero. `jq` is not a new dependency here:
# this image installs it (`docker/Dockerfile.chain`) and this script already decides with it.
tx_code() { # reads a `query tx` answer on stdin; prints its code, or NOTHING
  jq -r 'if type=="object" and ((.txhash // "") != "") then (.code // 0) else empty end' 2>/dev/null
}

          QCODE=$(printf '%s' "$QOUT" | tx_code)
          [ -n "$QCODE" ] && break
        done
        [ "$QCODE" = "0" ] && { ok=1; break; }
