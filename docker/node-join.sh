...
# BEING REACHABLE IS NOT THE SAME AS PUBLISHING THE PORT, AND NOTHING HERE USED TO SET THIS.
# A node advertises `tcp://0.0.0.0:26656` unless an external address is given. That string means
# "I listen on every local interface"; to anyone reading it from outside it is UNDIALABLE. Measured
# on this network: every node dialled out to the same host and accepted nobody, so the topology was
# a star with no lateral link — and each operator saw a healthy node, because from the inside it IS
# healthy. A node with one peer has ONE source for every consensus message: a missed proposal has
# no second place to come from.
# THREE STATES, and the empty one is the DEFAULT because it is CORRECT behind NAT: a wildcard is
# honest, a wrong address is not. Peers STORE what you advertise and RE-GOSSIP it, so a private or
# loopback address does not merely fail for you — it pollutes other operators' address books.
# CARRIER-GRADE NAT IS THE ONE AN OPERATOR CANNOT SEE FROM THE INSIDE. Mobile, fibre and satellite
# ISPs hand out 100.64.0.0/10 (RFC 6598, "shared address space"): `ip addr` shows an address that
# looks routable, it is not RFC 1918, and it is shared between thousands of subscribers, so nobody
# outside can dial it and no port forwarding on the customer's router can change that. Advertising
# it is the same damage as advertising 10.x — peers store it and re-gossip it.
# THE RANGE IS 100.64 THROUGH 100.127 ONLY: 100.63.x and 100.128.x are ordinary public addresses,
# so the lazy `100.*` would refuse operators who are genuinely reachable.
_EA="${DENDRA_EXTERNAL_ADDRESS:-}"
P2P_ARGS=""
if [ -n "$_EA" ]; then
  case "$_EA" in
    *:*) : ;;
    *) echo "[node] FATAL: DENDRA_EXTERNAL_ADDRESS='$_EA' must be host:port (the port peers dial)."; exit 1 ;;
  esac
  _EH="${_EA%:*}"; _EP="${_EA##*:}"
  case "$_EP" in
    ''|*[!0-9]*)
      echo "[node] FATAL: DENDRA_EXTERNAL_ADDRESS='$_EA' has no numeric port."; exit 1 ;;
  esac
  case "$_EH" in
    ''|0.0.0.0|127.*|localhost|10.*|192.168.*|169.254.*|172.1[6-9].*|172.2[0-9].*|172.3[01].*|100.6[4-9].*|100.[7-9][0-9].*|100.1[01][0-9].*|100.12[0-7].*|'::'|'[::]'|'[::1]')
      echo "[node] FATAL: DENDRA_EXTERNAL_ADDRESS='$_EA' is a wildcard, a loopback, a private or a"
      echo "[node]        carrier-NAT (100.64.0.0/10) address, so peers cannot dial it."
      echo "[node]        Peers store and re-gossip what you advertise, so this is WORSE than advertising"
      echo "[node]        nothing. Leave it empty unless the port is genuinely reachable from outside."
      exit 1 ;;
  esac
  P2P_ARGS="--p2p.external-address $_EA"
  echo "[node] advertising $_EA to peers — they can dial this node."
else
  echo "[node] no external address set: this node dials out but accepts nobody, so it is not a"
  echo "[node]        redundancy option for anyone else. Set DENDRA_EXTERNAL_ADDRESS once the P2P"
  echo "[node]        port is forwarded."
fi

echo "[node] starting (syncing from seeds)..."
# Unquoted on purpose: empty must expand to NO argument, not to an empty one.
# shellcheck disable=SC2086
exec dendrad start --home "$HOME_DIR" --minimum-gas-prices "0udndr" $P2P_ARGS
