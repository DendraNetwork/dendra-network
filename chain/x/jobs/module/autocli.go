	...
	return &autocliv1.ModuleOptions{
		Query: &autocliv1.ServiceCommandDescriptor{
			...
			RpcCommandOptions: []*autocliv1.RpcCommandOptions{
				...
				{
					RpcMethod: "ValidatorVrfKey",
					Use:       "validator-vrf-key [operator]",
					// The one check an operator could not run before bonding. A validator bonded without
					// an anchored key contributes nothing to the seed while every indicator says healthy.
					Short:          "VRF public key ANCHORED for an operator — anchored=false means no key, not a failed query",
				...
				},
				{
					...
					// NOT the jury: say so here, because the two names sit next to each other and one
					// was already read for the other. See `audit-committee` below.
					Short:          "WORK assignment of a job, in draw ORDER (members[0] is the PRIMARY) — NOT the audit jury, see audit-committee (ADR-034)",
					PositionalArgs: []*autocliv1.PositionalArgDescriptor{{ProtoField: "job_id"}},
				},
				{
					RpcMethod:      "AuditCommittee",
					Use:            "audit-committee [job-id]",
					Short:          "ANCHORED JURY of a job: the only list allowed to vote a verdict (ADR-032), with its seating height. --redo reads the re-adjudication jury (ADR-033)",
					PositionalArgs: []*autocliv1.PositionalArgDescriptor{{ProtoField: "job_id"}},
				},
				{
