...
func (p Params) Validate() error {
	...
	if p.VerificationMode == 1 {
		...
		if p.AuditResolveTimeout <= p.DisputeWindow {
			return fmt.Errorf("verification_mode=1 requires audit_resolve_timeout > dispute_window (otherwise the timeout resolves before adjudication)")
		}
		...
		//
		// Two sources of unpredictability exist, and AT LEAST one is required:
		//   - `committee_seed_source = 1`: the seed comes from the validators' aggregated VRF, so no
		//     single actor controls it;
		//   - `committee_reveal_delay > 0`: the seed stays empty at open time and is later fixed on the
		//     AppHash of a future block, unknown when the job is opened.
		//
		// With both at zero the seed is fixed at open time on `height:time`, two quantities the job
		// creator observes. It reopens until it draws the committee it wants. In redundant mode (k=3)
		// peer divergence limits the gain; in optimistic mode the audit committee IS the only
		// verification, so choosing it amounts to choosing the verdict. This is the anti-grinding
		// guard optimistic mode assumes it has, and nothing used to require it.
		if p.CommitteeSeedSource != 1 && p.CommitteeRevealDelay == 0 {
			return fmt.Errorf("verification_mode=1 requires an unpredictable committee seed: set committee_seed_source=1 (decentralised VRF) OR committee_reveal_delay>0 (deferred reveal); with both at zero the seed is fixed on the height:time the job creator observes, which allows jobId grinding")
		}
	}
