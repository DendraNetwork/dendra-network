...
func TestDeleteMinerRefusedOnAnchoredSeat_UndatedFallsBackToEligible(t *testing.T) {
	...
	require.NoError(t, f.keeper.AuditCommittee.Set(f.ctx, "jRepli", "mSansDate,mJ2"))
	require.NoError(t, gvSetJob(f, f.ctx, "jRepli", types.Job{
		JobId: "jRepli", State: "open+paid+optimistic+disputed", MinerId: "mOther",
	}))
