...
func (k Keeper) deferAuditForSmallJury(ctx context.Context, h, hPending int64, jobId string, eligibles, floor int, p types.Params) error {
	// ADR-044, gesture 2 — THE BOUND SITS AT THE DECISION, NOT AT THE CALLER. Put here rather than at
	// the single call site so that any future path reaching this deferral inherits it; a bound one
	// caller applies is a bound the next caller silently escapes. Dormant at `audit_unwind_blocks = 0`,
	// where `unwindStrandedRetention` returns before reading anything.
	if k.unwindStrandedRetention(ctx, h, jobId, p) {
		// Settled, not postponed: the pending entry is REMOVED rather than moved forward. Leaving it
		// would re-collect a job that no longer has a debt, at every block, for ever.
