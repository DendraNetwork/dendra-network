...
// livePublicParams reproduces the parameter set this test needs: committee_seed_source=1 and
// committee_reveal_delay=0.
// ⚠️ THIS IS NOT WHAT THE PUBLIC CHAIN RUNS, and saying it was cost a false public claim. Measured
// 2026-08-21: the chain serves committee_reveal_delay=1, and docker/entrypoint-chain.sh defaults
// DENDRA_COMMITTEE_REVEAL_DELAY to 1. A delay of 0 under verification_mode=1 is REFUSED by
// Params.Validate; this fixture only reaches it because Params.Set bypasses Validate. The zero is
// the starting point the test needs, never a description of the live network.
//
...
func livePublicParams() types.Params {
	...
	p.CommitteeRevealDelay = 0 // the subject of this test — NOT the live value (the chain serves 1)
	return p
}
...
func TestRevealDelayIsGovernableWithoutAReset(t *testing.T) {
	...
	require.NoError(t, err)
	require.Equal(t, uint64(0), before.CommitteeRevealDelay, "starting point of this fixture — not the live value")
...
func TestRevealDelayChangesOpenJobBehaviourLive(t *testing.T) {
	...
	// `livePublicParams()` carries the real `audit_min_quorum`, so `OpenJob` now refuses to freeze a
	// pool that could never reach a verdict (ADR-042). This test is about the REVEAL DELAY, not about
	// the pool floor: the population below is what its subject presupposes and never had to state
	// while opening a job was unconditional. Registering them here keeps the measured behaviour intact
	// — a job that opens — rather than lowering the floor to keep the fixture as it was.
	for i := 0; i < 8; i++ {
		id := fmt.Sprintf("rdMiner%d", i)
		require.NoError(t, f.keeper.MinerRegisteredHeight.Set(f.ctx, id, 1))
		require.NoError(t, f.keeper.Miner.Set(f.ctx, id, types.Miner{MinerId: id, Stake: 1_000_000}))
	}
