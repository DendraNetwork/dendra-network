...
type Keeper struct {
	...
	// WorkCommittee -- the WORK committee, anchored as soon as it is derivable (ADR-045, 12). A store
	// DISTINCT from AuditCommittee: three consumers read every entry of that one as a JURY SEAT.
	WorkCommittee collections.Map[string, string]
	...
	// PendingJobExpiry indexes the jobs whose client escrow returns if nobody serves them (see keys.go).
	PendingJobExpiry collections.KeySet[collections.Pair[int64, string]]
