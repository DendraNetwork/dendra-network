...
// auditMaxPerBlock — HOW MANY due audit entries a single EndBlock may take, on each of the three
// queues. The three passes below used to walk the EXACT height with no cap, which was survivable only
// while a batch could not exceed one block's worth of settlements. `auditDeferStride` breaks that on
// its own: deferring by a FIXED stride makes every batch of the same residue class land on the SAME
// height and MERGE, so a persistently undecentralized seed (or a persistently short jury) grows the
// per-block batch without bound -- the identical defect already measured and closed on the reveal
// pass (`revealMaxPerBlock`, job_expiry.go). Capping alone would strand entries, so the range is
// widened at the same time to cover every DUE height (`K1 <= h`): what does not fit stays in the
// store and the next block takes it, oldest first. The two changes are one gesture; either without
// the other is a defect.
const auditMaxPerBlock = 200
...
func (k Keeper) runOptimisticAudit(ctx context.Context, h int64, seed string) error {
	...
	if err := k.PendingAudit.Walk(ctx,
		...
		func(key collections.Pair[int64, string]) (bool, error) {
			due = append(due, key)
			return len(due) >= auditMaxPerBlock, nil
		}); err != nil {
		return err
	}
	if len(due) == 0 {
	...
	if _, decentralized := k.committeeBaseSeedSourced(ctx, seed); !decentralized && p.CommitteeSeedSource == 1 {
		...
		for _, key := range due {
			// The key is removed BY ITS OWN HEIGHT, which is no longer necessarily `h`: the walk now
			// covers every due height, so an entry left behind by a capped block carries an older one.
			if err := k.PendingAudit.Remove(ctx, key); err != nil {
				return err
			}
			if err := k.PendingAudit.Set(ctx, collections.Join(h+auditDeferStride, key.K2())); err != nil {
				return err
			}
		}
		...
		if job, jErr := k.Job.Get(ctx, jobId); jErr == nil && jobIsPaid(job.State) && !jobIsResolved(job.State) {
			...
			if alreadySelected || auditDraw(seed, jobId) < k.effectiveAuditBps(ctx, p, job) {
				...
				if floor := effectiveSlashFloor(p); len(members) < floor {
					...
					if err := k.deferAuditForSmallJury(ctx, h, key.K1(), jobId, len(members), floor, p); err != nil {
						return err
					}
					continue
				}
...
func (k Keeper) runAuditResolveTimeout(ctx context.Context, h int64) error {
	...
	var due []collections.Pair[int64, string]
	if err := k.PendingAuditResolve.Walk(ctx,
		new(collections.Range[collections.Pair[int64, string]]).
			EndExclusive(collections.PairPrefix[int64, string](h+1)),
		func(key collections.Pair[int64, string]) (bool, error) {
			due = append(due, key)
	...
	for _, key := range due {
		if err := k.resolveDisputedAudit(ctx, h, key.K2(), p, true); err != nil { // allowAppeal=true: may defer to an appeal
			return err
		}
		if err := k.PendingAuditResolve.Remove(ctx, key); err != nil {
			return err
		}
	}
	return nil
...
func (k Keeper) runAppealResolveTimeout(ctx context.Context, h int64) error {
	...
	var due []collections.Pair[int64, string]
	if err := k.PendingAppealResolve.Walk(ctx,
		new(collections.Range[collections.Pair[int64, string]]).
			EndExclusive(collections.PairPrefix[int64, string](h+1)),
		func(key collections.Pair[int64, string]) (bool, error) {
			due = append(due, key)
	...
	for _, key := range due {
		if err := k.resolveDisputedAudit(ctx, h, key.K2(), p, false); err != nil { // allowAppeal=false: last instance
			return err
		}
		if err := k.PendingAppealResolve.Remove(ctx, key); err != nil {
			return err
		}
	}
	return nil
...
func (k Keeper) resolveDisputedAudit(ctx context.Context, h int64, jobId string, p types.Params, allowAppeal bool) error {
	...
	if jobIsOptimistic(job.State) && job.MinerId != "" {
		...
		default:
			...
			// ADR-044, gesture 2 — BEFORE the appeal branch, and the order matters. An appeal is one
			// more postponement; granting it on a debt already older than `audit_unwind_blocks` is
			// exactly the unbounded deferral this bound exists to end. Dormant at 0, where
			// `unwindStrandedRetention` returns before reading anything. On success the job has been
			// resolved and PERSISTED by the helper, so we return at once: the `job` held here is stale
			// from that instant and must not be written back over it.
			if k.unwindStrandedRetention(ctx, h, jobId, p) {
				return nil
			}
