...
func TestADR025OptimisticAuditEndBlock(t *testing.T) {
	...
	// REGISTERED AT BLOCK 4, THE JOB OPENS AT 5. A job freezes its pool on the block BEFORE its own
	// (`frozenPoolAnchor`), so a miner appearing in the opening block is not in that pool -- that is
	// the rule, not an accident of this fixture. Placing the registration one block earlier is also
	// what an operator does: it registers, and clients open jobs afterwards. This case observes the
	// audit DRAW, so it is given a miner the draw can actually reach.
	idM1 := deriveIDFor(t, f, creator)
	_, err = srv.CreateMiner(sdk.UnwrapSDKContext(f.ctx).WithBlockHeight(4),
		&types.MsgCreateMiner{Creator: creator, MinerId: idM1, Operator: creator, Stake: 1000})
	require.NoError(t, err)
