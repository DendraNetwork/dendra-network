...
func (k msgServer) SettleSemantic(ctx context.Context, msg *types.MsgSettleSemantic) (*types.MsgSettleSemanticResponse, error) {
	...
	// ANTI-DOUBLE-SLASH: a job whose k=3 committee has ALREADY been judged — by ANY of the three
	// redundant paths — must not go through SettleSemantic. Anchored commits are IMMUTABLE and therefore
	// still divergent, so it would re-slash the SAME outliers; every one of these handlers is
	// permissionless, so the penalty compounds.
	//
	// ⛔ THIS TEST USED TO NAME ONE MARKER, "finalized", AND THAT LEFT AN EDGE OPEN. The justification
	// read: "TARGETED at this handler alone, so the documented flow FinalizeJob -> Payout stays open".
	// Payout is a different handler and this test never touched it, so nothing was bought by the
	// narrowing — while "+verified", written by VerifySemantic, matched neither this test nor jobIsPaid.
	// MEASURED before changing anything (compound_slash_verify_then_settle_test.go): verify then settle
	// took 1000 -> 200 -> 40, i.e. ~96 % of an honest miner's bond for one piece of work, on two calls
	// anyone can make. `jobK3Adjudicated` is the predicate the other two paths already ask
	// (msg_server_finalize_job.go, msg_server_verify_semantic.go) and its own doc-comment describes this
	// exact compounding — the rule was written down and applied in two files out of three.
	// The exit stays open: Payout guards only on jobIsPaid, so a judged-but-unpaid job is still paid
	// ONCE, exactly as after FinalizeJob. Verified by reading its guards, not assumed.
	// Placed before the verification_mode branch, so it covers both modes.
	if jobK3Adjudicated(job.State) {
		return nil, errorsmod.Wrap(sdkerrors.ErrInvalidRequest, "k=3 committee already judged; pay through Payout")
	}
