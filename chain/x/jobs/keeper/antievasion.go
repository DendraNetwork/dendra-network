...
func (k Keeper) auditVerdictTally(ctx context.Context, jobId, primId string) (cheated, valid bool, voters, invalidVotes, seats, distinctOps, maxOpVotes int) {
	...
	// THE DENOMINATOR COUNTS SEATS THAT CAN STILL VOTE, NOT SEATS THAT WERE ONCE DRAWN.
	//
	// It used to be `len(allowed)` — the committee as ANCHORED at draw time. But a verdict below only
	// counts when `k.Miner.Get(mid)` succeeds (a registered miner is required, a few lines down), and a
	// juror can leave the registry after the anchor: eviction for inactivity, or a delete. The
	// denominator then stays at its draw-time size while the number of votes that can ever be cast has
	// dropped, so `auditRelativeBar` asks for a share of a committee that no longer exists. The bar
	// becomes unreachable, the audit defers for ever, and the held fee and the dispute bond stay locked
	// with it — an outcome nobody chose and nothing reports.
	//
	// SHRINKING THE DENOMINATOR IS SAFE HERE, AND IT IS WORTH SAYING WHY, because the obvious fear is
	// that an attacker silences jurors to lower the bar and slash an honest primary cheaply. Two
	// independent floors forbid it: `auditVindicateDecision` requires `voters >= effectiveSlashFloor(p)`
	// — an ABSOLUTE count that does not depend on seats at all — and `auditRelativeBar` starts from that
	// same floor as its lower bound before taking ⌈2/3 × seats⌉. Removing seats can only reduce the
	// RELATIVE part, never take the decision below the absolute floor.
	seats = 0
	for mid := range allowed {
		if _, mErr := k.Miner.Get(ctx, mid); mErr == nil {
			seats++
		}
	}
