...
// The age SURVIVES migration (exported as elapsed blocks, re-anchored on import, clamped on the old
// side). A debt that grew younger by migrating would silence the alarm exactly when it matters.
func TestGenesisHeldSinceTravelsAsAge(t *testing.T) {
	...
	require.NoError(t, err)
	require.GreaterOrEqual(t, resp.OldestAgeBlocks, uint64(29),
		"after import the age keeps GROWING (clamped on the old side): the alarm does not rejuvenate through migration")
}
