...
func (k Keeper) drawAuditCommittee(ctx context.Context, seed, jobId, primaryId, disputerId string) ([]string, error) {
	...
	// PARAMS READ ONCE, BEFORE THE WALK. `eligibleAsJuror` reads them itself, so leaving it inside the
	// loop meant one store read per registered miner — and this draw runs once per job settled in the
	// block, in an EndBlocker with no gas. The window is the only thing the predicate needs from them.
	// Unreadable params keep the compiled constant, exactly as the predicate does on its own.
	window := jurorFreshnessBlocks
	prm, prmErr := k.Params.Get(ctx)
	if prmErr == nil {
		window = effectiveJurorFreshness(prm)
	}
	...
	err := k.Miner.Walk(ctx, nil, func(key string, m types.Miner) (bool, error) {
		...
		if !k.eligibleAsJurorInWindow(ctx, key, h, window) {
			return false, nil
		}
	...
	if prmErr == nil && prm.AuditMinQuorum > asUint64(size) {
		size = int(prm.AuditMinQuorum)
	}
