...
// to prevent.
func TestCreateCommitRefusesWhenParamsAreUnreadable(t *testing.T) {
	f := initFixture(t)
	...
	// THE JOB EXISTS FIRST, and it is required for this case to still measure what it aims at: the
	// existence guard refuses BEFORE the params read, so without the job this test would see the
	// wrong refusal and vouch for a fail-closed it never exercised.
...
func TestOpenJobRefusesWhenParamsAreUnreadable(t *testing.T) {
	f := initFixture(t)
	srv := keeper.NewMsgServerImpl(f.keeper)
...
func TestReadableParamsBothPathsPass(t *testing.T) {
	f := initFixture(t)
	seedJurorPool(t, f, 5)
	srv := keeper.NewMsgServerImpl(f.keeper)
	client, err := f.addressCodec.BytesToString([]byte("clientOuvreur_______________"))
