...
// DeleteDecentralizedSeedContributors prunes an OLD height (sliding anti-bloat window, cf. V8-N1).
//
// THE PRUNING COMMENT NAMED FOUR COLLECTIONS AND THE CODE DELETED THREE. This one was the missing
// name: the contributor COUNT was written at every height and never removed, so it grew for the life
// of the chain while its three neighbours were bounded -- roughly one entry per block, forever, in the
// one collection whose absence nothing would ever report. A comment is not a deletion.
func (k Keeper) DeleteDecentralizedSeedContributors(ctx context.Context, height int64) error {
	return k.DecentralizedSeedContributors.Remove(ctx, height)
}
