...
func TestREADMEClaimCAucunAncrageAucunSlashDur(t *testing.T) {
...
func TestReadjudicationEgaliteNeSlashePasNiNeVindique(t *testing.T) {
	...
	jouer := func(t *testing.T, jobId, seed string, dAccord int) (*fixture, error) {
		f, srv, disp := adr033Setup(t, seed)
		// Four large stakes outside the panel: they take the three seats of the ORIGINAL committee,
		// so no respondent is excluded by the anti-collusion guard. Verified below, not assumed.
