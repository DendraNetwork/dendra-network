...
func (k Keeper) InitGenesis(ctx context.Context, genState types.GenesisState) error {
	// NO INIT PATH MAY INSTALL AN UNVALIDATED GENESIS, and `validate-genesis` is not an init path.
	//
	// `GenesisState.Validate()` is what `dendrad genesis validate-genesis` runs, but nothing forces a
	// chain to be started through that tool: a hand-made, imported or third-party genesis reaches this
	// function directly. Two states then install without a word, and neither can be left afterwards.
	//   - `audit_judge_model` containing whitespace. `Params.Validate` refuses it; installed, it is read
	//     by the judge worker and passed as `--model-id` on every verdict, so with the registry enforced
	//     `IsActive` is false for that string and NO audit ever concludes.
	//   - a model with an EMPTY or DUPLICATE id. Installed, `ExportGenesis` re-emits it, so the chain
	//     can no longer produce a genesis that its own validator accepts.
	//
	// It refuses exactly what the validator refuses and nothing more: a model with no `weights_sha256`
	// still loads, deliberately, for the reason stated below.
	if err := genState.Validate(); err != nil {
		return fmt.Errorf("invalid modelregistry genesis (refused at load time): %w", err)
	}
