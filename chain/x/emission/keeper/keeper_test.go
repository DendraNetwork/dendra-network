...
// mockEmBank — an IN-MEMORY bank used to test `RunEpoch`. With `bank=nil` in the fixture, NO test
// called `RunEpoch`, so neither the `balance == Reserve + WorkPool + AvailPool` invariant nor the coin
// movement was covered by anything. `supply` drives demand (a drop in supply); `moved` records what
// leaves for the fee_collector; `fail` simulates an unfunded module account.
