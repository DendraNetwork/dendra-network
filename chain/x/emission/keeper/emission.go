...
// EpochRelease releases ONE epoch from `reserve` (udndr). `nonRecDemand` (udndr) CAPS the work
// subsidy, so a miner cannot subsidise itself beyond the demand it does not get back.
//
// WHAT THE CALLER PASSES IS THE DROP IN SUPPLY, i.e. the FEE BURN ALONE (`RunEpoch`, epoch.go).
// That is narrower than "everything a miner cannot refund to itself", and the difference is not a
// rounding detail: the treasury and dev shares MOVE coins between accounts rather than destroying
// them, so they leave total supply untouched and cannot appear in a supply delta at all.
//
// The narrow reading is deliberate — a supply delta needs no cross-module jobs->emission wiring, and
// with minting at zero the supply can only fall — but anything sized on the wider one over-estimates
// this cap by the treasury and dev shares, roughly a factor of 2.5 on the flow that pays miners. So
// any model or document quoting this gate must quote THE SUPPLY DELTA, not the fee split.
