...
func TestEmissionRoundTripPreserveLaPresence(t *testing.T) {
	for _, cas := range []struct {
		...
		t.Run(cas.nom, func(t *testing.T) {
			...
			b := initFixture(t)
			// THE BANK FOLLOWS THE COUNTERS. `balance(emission module) == Reserve + WorkPool +
			// AvailPool` is the invariant RunEpoch states and the genesis guard enforces, so a fixture
			// that imports credited pools without holding the coins models a chain that cannot exist --
			// and the guard rightly refuses it. The balance is DERIVED from the state being imported
			// rather than retyped, so a case that credits different amounts stays covered.
			if st := exporte.State; st != nil {
				b.bank.spendable = st.Reserve + st.WorkPool + st.AvailPool
			}
...
func TestEmissionHasNoGhostPrefix(t *testing.T) {
