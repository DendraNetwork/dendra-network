...
Example:
	dendrad multi-node --v 4 --output-dir ./.testnets --validators-stake-amount 1000000,200000,300000,400000 --list-ports 47222,50434,52851,44210
	`,
...
func initTestnetFiles(
	...
	for i := 0; i < args.numValidators; i++ {
		...
		// SECRET: this file carries the mnemonic in clear text — whoever reads it controls the
		// validator's account. It is therefore written 0600 (owner only) rather than 0644, which exposed
		// it to every account on the machine. These directories are also mounted as container volumes,
		// where "every account on the machine" is a population one does not choose.
		file := filepath.Join(nodeDir, fmt.Sprintf("%v.json", "key_seed"))
		if err := writeFile(file, nodeDir, cliPrint, 0o600); err != nil {
			return err
		}
		...
		// COMMISSION — realistic rates, not 100 %.
		//
		// The scaffold set (1, 1, 1): a 100 % commission, a 100 % cap and a 100 % maximum change per
		// day. A delegator of such a validator receives NOTHING, and the validator can move from 0 to
		// 100 % in one transaction. On a testnet meant to replay the real network's incentives, that
		// makes everything depending on delegation rewards unobservable — and these values get copied
		// verbatim into operator runbooks.
		commission := stakingtypes.NewCommissionRates(
			math.LegacyNewDecWithPrec(10, 2), // rate 10 %
			math.LegacyNewDecWithPrec(20, 2), // cap 20 %
			math.LegacyNewDecWithPrec(1, 2),  // max change 1 %/day
		)
		...
		// PUBLIC artefact: a gentx is meant to be collected and redistributed (it carries only signed
		// data, no secret).
		if err := writeFile(file, gentxsDir, txBz, 0o644); err != nil {
			return err
		}
...
// writeFile writes `contents` to `file`, creating `dir` if needed. The permission is an ARGUMENT
// rather than a constant: this same helper writes public artefacts (gentx files, meant to be
// distributed) and a SECRET (the validator account's mnemonic). A single permission for both can only
// be correct for one of them, and it is the secret that loses.
func writeFile(file, dir string, contents []byte, perm os.FileMode) error {
	if err := os.MkdirAll(dir, 0o755); err != nil {
		return fmt.Errorf("could not create directory %q: %w", dir, err)
	}

	if err := os.WriteFile(file, contents, perm); err != nil {
		return err
	}
...
func collectGenFiles(
	...
	for i := 0; i < numValidators; i++ {
		...
		// RPC on LOOPBACK: this is the only place where the listen address reaches `config.toml`,
		// written a few lines below. RPC is an unauthenticated administration interface; opening it on
		// every interface would expose the nodes of a local testnet to the whole network. The P2P
		// listener above, by contrast, MUST listen everywhere — that is its job.
