...
func initAppForTestnet(app *app.App, args valArgs) *app.App {
	...
	// Fund local accounts.
	// F3 (2026-07-10) : x/mint est DÉ-ENREGISTRÉ de l'app (zéro mint structurel) → le compte de module
	// « mint » n'existe plus. Cet outil DEV in-process (jamais le réseau réel : l'offre PROD est allouée
	// au genesis) passe par ibctransfer, l'UNIQUE compte Minter restant (vouchers IBC, standard SDK).
