...
import (
	...
	"github.com/DendraNetwork/dendra-network/chain/app"
)
...
func initAppForTestnet(app *app.App, args valArgs) *app.App {
	...
	// Fund local accounts.
	// x/mint is DE-REGISTERED from the app (structurally zero mint), so the "mint" module account no
	// longer exists. This in-process DEV tool — never the real network, whose production supply is
	// allocated at genesis — goes through ibctransfer, the ONLY remaining Minter account (IBC vouchers,
	// standard SDK).
	for _, accountStr := range args.accountsToFund {
		handleErr(app.BankKeeper.MintCoins(ctx, ibctransfertypes.ModuleName, defaultCoins))

		account, err := app.AuthKeeper.AddressCodec().StringToBytes(accountStr)
		handleErr(err)

		handleErr(app.BankKeeper.SendCoinsFromModuleToAccount(ctx, ibctransfertypes.ModuleName, account, defaultCoins))
	}
