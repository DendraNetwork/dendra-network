...
// genesisLivres — the COMMITTED genesis files whose supply is locked here. A genesis file added to
...
var genesisLivres = []string{"config.yml", "config.optimistic.yml"}
...
func TestOffreFixeAucunModuleDeFrappe(t *testing.T) {
...
func TestOffreFixePermissionsDeModuleEpinglees(t *testing.T) {
...
func TestOffreFixeGenesisVautDixMillions(t *testing.T) {
	...
	sort.Strings(noms)
	attendus := append([]string{}, genesisLivres...)
	...
	require.Equal(t, attendus, noms,
		"the list of committed genesis files has changed: an unlisted genesis is covered by NO supply "+
			"check. Add the file to `genesisLivres`, or remove it from the repository.")
