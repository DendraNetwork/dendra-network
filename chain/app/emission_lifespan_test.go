...
// budget evaporates during its own testnet. `TestEmissionLifespanWitnessShortSettingUnderTheAssumption`
...
// the arbitration happens on numbers, with the bps-per-epoch-length table in hand. That
// table lives in docs/TOKENOMICS-CALIBRATION.md, in the development repository, and is not
// part of a published tree: from a published clone the numbers to arbitrate on are the
// ones this test prints, not a document to open.
const (
...
// TestEmissionLifespanOfShippedGenesisUnderTheAssumption — the load-bearing check. The suffix is not
...
func TestEmissionLifespanOfShippedGenesisUnderTheAssumption(t *testing.T) {
			...
			for _, regime := range []struct {
				...
				require.GreaterOrEqualf(t, annees, demiVieMinAnneesSousHypothese,
					"%s / %s: the Reserve halves in %d epochs of %d blocks = %.2f years, below the announced "+
						...
						"the bps-per-epoch-length table -- docs/TOKENOMICS-CALIBRATION.md, which lives in "+
						"the development repository and not in a published tree -- then move this "+
						"bound deliberately.\n%s",
...
// TestEmissionLifespanDemandAcceleratesTheCurve — the ordering between the two regimes is a
...
func TestEmissionLifespanDemandAcceleratesTheCurve(t *testing.T) {
	for _, nom := range shippedGenesis {
		chemin := filepath.Join("..", nom)
		p := paramsEmissionDuGenesis(t, chemin)
		reserve := reserveDuGenesis(t, chemin)
...
// TestEmissionLifespanWitnessShortSettingUnderTheAssumption — THE WITNESS. A band is only worth what it
...
// not its answer.
func TestEmissionLifespanWitnessShortSettingUnderTheAssumption(t *testing.T) {
...
// TestEmissionLifespanIsNotInertUnderTheAssumption — the mirror failure. Integer truncation
...
func TestEmissionLifespanIsNotInertUnderTheAssumption(t *testing.T) {
	for _, nom := range shippedGenesis {
		t.Run(nom, func(t *testing.T) {
			chemin := filepath.Join("..", nom)
			p := paramsEmissionDuGenesis(t, chemin)
...
// TestEmissionLifespanAvailabilityCadenceIsIndependent — `emission.epoch_blocks` and
...
func TestEmissionLifespanAvailabilityCadenceIsIndependent(t *testing.T) {
	for _, nom := range shippedGenesis {
		chemin := filepath.Join("..", nom)
