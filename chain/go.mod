module github.com/DendraNetwork/dendra-network/chain

go 1.25.10

// The exact toolchain that builds a release binary is a property of the repository, not of the
// machine that happens to run the build. Without this line the toolchain is whatever the
// contributor has installed, and two builds of the same tag can differ; `actions/setup-go` also
// reads it, so CI and a local build agree by construction. Any released artifact whose hash is
// meant to be checked must be built by the toolchain named here.
toolchain go1.26.5
