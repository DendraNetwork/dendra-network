...
**Status:** Decided by the project owner, **in force through genesis**. The values fixed here are
served by the running chain — `signed_blocks_window` 10000 and `min_signed_per_window` 0.85 — because
the genesis paths write them, so no governance vote is needed for them to apply. Read them rather
than this line: `curl -s http://api.dendranetwork.com:1317/cosmos/slashing/v1beta1/params`.
...
identical in `docker/entrypoint-chain.sh` (the `DENDRA_SLASH_*` defaults it applies) and in the
`slashing.params` blocks of `chain/config.yml` and `chain/config.optimistic.yml`. The five keys
are `signed_blocks_window`, `min_signed_per_window`, `downtime_jail_duration`,
`slash_fraction_double_sign` and `slash_fraction_downtime` - grep those rather than a line number,
...
The genesis path builds its strings by integer arithmetic (`docker/entrypoint-chain.sh::_dec`)
