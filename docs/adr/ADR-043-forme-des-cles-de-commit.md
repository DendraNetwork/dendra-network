...
**Status:** Accepted, implemented, **in force**. The `5 → 6` bump this decision reserved never
happened: `docker/CONSENSUS_EPOCH` carries no epoch 6 entry and stands at **13**. The change reached
the network through a fresh genesis rather than through that bump, and the refusal it decides is in
the tree — `chain/x/jobs/keeper/msg_server_commit.go`, the fail-closed `Job.Has` branch.
**Not measured:** the RPC reports an application version of `dev`, so no query confirms which commit
the network runs; what is measured is the tree and the epoch ledger.
