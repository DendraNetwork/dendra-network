...
does not move. A score moves under its holder: measured on the live chain at a time when a single miner was registered,
it scored 80 % with all three anti-sybil caps already firing, and dropped to **zero** when the only client
address was excluded, because the denominator starves. Both figures are a snapshot of that shape, not of
the network today - the registry is read, never repeated: `/dendra/jobs/v1/miner`. No ranking is published until the index counts
