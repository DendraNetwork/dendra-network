...
**(b) A silent juror is sanctioned by no path.** `silence_slash_bps` is read in exactly one place: the
`muteSignal` branch of `antievasion.go::clawbackPayment`. That function has **no production caller** -
the "no quorum then clawback" path it belonged to was replaced by deferral, and the comment heading the
function says so. The parameter is **inert**, not dormant - the distinction `params_inert_test.go`
draws: dormant is a value, inert is an absence of code. The shipped genesis says as much on the field
itself (`chain/config.optimistic.yml`, the `silence_slash_bps` entry and the comment above it), so a
governance vote raising it would pass and change nothing.
...
wide for precisely this reason. ⚠️ It is a **block** window and it is **governable**: the value in
force is read from the chain, not from this record, and a figure in days would drift with the block
interval the consensus does not fix.
