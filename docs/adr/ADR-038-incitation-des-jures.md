...
`Demand` is credited only at settlement, only to the primary (`msg_server_settle_job.go:86`,
`msg_server_settle_semantic.go:274`). A juror's `Demand` never moves, so a juror's subsidy cap never
rises.
...
**(i) The predicate that already exists does not bite.** `eligibleAsJuror` (`miner_vitality.go:280`)
