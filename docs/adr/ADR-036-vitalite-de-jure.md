...
> ⚠️ **The network this record measures no longer exists, and it carried no date.** The figures
> below (`8 registered miners, 5 voters, 3 silent`) were read around **2026-07-27**, the date this
> record was written. That chain was destroyed and relaunched on a fresh genesis on **2026-08-16**,
> and again on **2026-08-22** (current genesis, epoch 13). Read the live count rather than these
> lines: `/dendra/jobs/v1/miner` publishes the registrations, and it is the only reading that
> cannot go stale. The mechanism described below is unchanged in the code, which is why this record
> is kept rather than retired. *A measured figure with no anchor reads as a reading of today.*
