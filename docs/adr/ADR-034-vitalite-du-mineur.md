...
4. **Eviction of the inert** (once per **epoch** — the unit the chain counts; a figure in days would
   drift with the block interval): **objective** (an on-chain criterion
...
- **Direct consequence**: fixing F1 **alone** closes nothing — it converts a **free** attack into an
  attack costing **one transaction per freshness window**. Vitality would then measure willingness to
  pay gas, not production. (The window is a **block** count, governable — see the Open point below.)
...
- **Open — both windows are CHOICES, not measurements.** They are counted in **blocks**
  (`jurorFreshnessBlocks`, and the eviction window beside it), never in days: the consensus does not fix
  a block interval — it emerges from `timeout_commit` and propagation — so any figure in days is an
  observation that drifts, and this record used to carry one. ⚠️ **Both are governable, and the juror
  window deployed on the network has since been widened well past the figure this record was written
  against: read it from the chain, never from here.** Too short and an outage gets billed to the honest
  party. The measurement that
