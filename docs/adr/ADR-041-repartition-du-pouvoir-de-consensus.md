...
> validator had been lost, so a single validator carried **100 %** of the voting power and the VRF contributor
> count was back to **1** against a floor of 2. **That is a snapshot, not the current split** - read it rather
> than this line: `/cosmos/staking/v1beta1/validators` and `/dendra/jobs/v1/committee_seed_health`.
> The decision itself is unchanged — the target is now **two**
...
**5.02 s → 5.23 s per block**, about 4 %, on the pair of samples taken at the change. Four later
samples say that pair understates the price, and that the price is not a point value. Each sample
below is 120 contiguous heights, wall-clock, taken from block headers:

| regime | samples, seconds per block |
|---|---|
| one effective voter | 5.02 · 5.03 |
| more than one effective voter | 5.23 · 5.34 · 5.36 · 5.36 · 5.74 |

The gap between the two regimes is therefore about **0.3 s per block, near 6 %** — not the two tenths
the first pair suggested — and the multi-voter regime is a **range**, not a value. It is measured in
both directions: the 5.03 s sample was taken while the network had fallen back to a single effective
voter, and the price came back when the partners did — the three highest samples were taken with
three bonded validators, two of them carrying the weight. *A quantity that moves back when its cause
moves back is measured; one that does not move was not.*

⚠️ These are wall-clock durations, and the consensus does not fix them: they emerge from
`timeout_commit` and from propagation. They are cited here because the record is about a cost paid in
real time, and they are stated as a range for exactly that reason. Anything that must be exact is
counted in blocks.

`round 0` on 100 % of blocks **in both regimes**: the consensus never had to retry, before or after.
