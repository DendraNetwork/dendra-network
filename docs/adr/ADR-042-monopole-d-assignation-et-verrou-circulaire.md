...
6. **Bound the client's wait, end to end.** ⚠️ **Re-read in the code on 2026-09-08, and this record's
   own arithmetic was wrong.** The bounded waits on the client path are `wait_tx` (24 polls x 2 s =
   **48 s**), `assigned_committee` (**45 s**) and the response deadline inside `quick_metered` /
   `quick` (**240 s**, overridable by `DENDRA_TIMEOUT`). They *compose*, because the first two run
   before the deadline is armed: the bound is **~333 s**, not the "roughly nine minutes" first stated
   here. The MEASUREMENT was right ("no response after 320 s") and the arithmetic never agreed with
   it — *a stated composition that contradicts its own measurement is a claim nobody re-derived.*
   ⚠️ `wait_beacon` (45 s) is **no longer on the path and has no caller at all**: the committee
   query's own polling replaced it, and the function survives its purpose. A bounded wait that reads
   as live and is dead misleads the next reader about where the time goes.
   The honest failure message exists and still arrives far too late to be honest in practice.
