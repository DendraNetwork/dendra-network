...
**Status:** **Proposed.** One half is a governance parameter and the other is consensus-breaking Go
that does not exist yet. Neither should be enacted before the owner arbitrates, and the two have a
**mandatory order** (below) that reverses the outcome if inverted.
...
The deferral is unbounded *on purpose* — `audit_jury_floor.go:41`: bounding it would make evasion
...
(`audit_jury_floor.go:53`) and the `default` arm of `resolveDisputedAudit` — if the parameter is
...
**Why this is not the release the code forbids.** `audit_jury_floor.go:41` — *"releasing for want of
