...
**Escrow, as a property.** `hold_bps` sits at its maximum in the launch genesis, so a miner's net payment is **retained at settlement** and resolved at the job's audit checkpoint. A job the audit lottery does not select is **released to the miner** there (`audit_sampling.go`, the `releaseHeld` branch) — at the genesis `audit_sample_bps = 5000` that is about half of jobs, a governable figure to be read from the chain rather than from this page. A job it does select waits for the audit layer, which on the current endpoint draws committees and cannot make one conclude for want of jurors (§8), so its fee stays **held**.

Held means **retained, not cancelled and not expired**. The deferral has no timeout and no attempt counter, by design: releasing a payment because the audit could not run would let an abstaining validator push jobs through unverified, and cancelling it would punish a miner for someone else's absence. The backlog drains in two stages and the first has started: the seed floor is now met on part of the blocks, so unselected jobs finalise and release there. What remains held is what the lottery selected, and that waits on **four more miners**: an audit resolves — in either direction — only when at least `audit_min_quorum` jurors vote, that parameter is **4** on this chain, and the jury excludes the miner being judged. Operators arriving, not a patch. `dendrad query jobs held-summary -o json` reports the retained total and the **age of the oldest retention**, which is what makes "retained, not lost" a falsifiable claim rather than a reassurance.
...
**Publicly reachable today:** an **open devnet endpoint** — blocks, a queryable API, a published and verifiable genesis, and a deployment kit a third party can run. It carries **three validators and one miner**; two validators and the miner are operated by the project, and the third validator is an outside operator — a count, not a decentralisation: it bonds 9 DNDR while the largest validator holds **99.9986 %** of the voting power, so fault tolerance is still zero. Inference **is** served there and jobs settle. What the endpoint does **not** do is verify, and the reason is now the second of the two floors rather than the first: committees **are** drawn, on the blocks that meet the VRF contributor floor, and not one can reach a verdict: the jury is every registered miner except the one under audit, a single miner leaves it empty, and even a jury that exists concludes only when `audit_min_quorum` = 4 of its members vote — so five registered miners, not two, is the real floor (`committee-seed-health`, `audit-deferred`, `list-miner`, `params` — queryable rather than asserted). It is an endpoint to read, join and break, and it is not a network of verified inference. That clears on its own as operators arrive; it requires no code or genesis change.

**A third condition, which does not clear on its own.** The Reserve described in §6 exists as an *allocation* — 3.3 M DNDR, readable in the genesis — but the account the release path draws from is a **module account, and it is empty**: the allocation sits in an ordinary genesis account instead. Both halves are queryable rather than asserted (`dendrad query bank balances <emission module address>`, and the genesis `app_state.bank.balances`). The consequence is bounded and worth stating exactly: at each epoch the module computes a release, attempts the security transfer, fails, and **skips that epoch without loss** — the Reserve counter and the pools are left untouched, the chain does not halt, and nothing is emitted. So the settlement path funded by **client fees** works as described, while the **emission** path is inert until the module account is funded. Funding it is a deliberate, one-way transfer of a third of the supply into a module account, and it has not been made.
