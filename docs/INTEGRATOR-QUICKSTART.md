...
>
> **What follows describes the protocol. Whether any given stage has run is a READING, not a property
> of this page** - so this page states none, and hands you the query instead. Counts and "has never
> happened" claims go stale silently; a command does not.
>
> `audit_state` is `"pending"` in every response - always, by construction and not as a passing
> state: the gateway writes that field as a constant and never reads it back (see the note in
> section 2). To learn what actually happened to a job, read the chain, never the response.
>
> **Three stages gate a verdict, and they are ordered.** A job must be OPENED; the audit lottery must
> DRAW a committee for it; that committee must CONCLUDE. Each has its own condition, and none of them
> is a miner count:
> - **Opening** is refused when too few miners are eligible at that block -
>   `chain/x/jobs/keeper/pool_freeze.go::enoughEligibleMinersToVerify` needs `audit_min_quorum` + 1,
>   because the jury excludes the miner under audit.
> - **Drawing** is gated on the seed: under `committee_seed_source=1` no committee is drawn while
>   `latest_contributors` stays under `committee_min_vrf_contributors` - the jobs are deferred and
>   their fees stay held (`x/jobs/keeper/audit_sampling.go`, which logs `no decentralized VRF seed at
>   this height -> NO audit draw`). Only a bonded validator anchoring a VRF key raises that reading.
> - **Concluding** needs `audit_min_quorum` jurors to vote, and voting is gated on hardware: a
>   registered miner that never runs `--judge` casts nothing.
>
> Read all three rather than assuming which one is closed:
> ```
...
> ```
>
> Write the `vindicated` / `rejected` branches of section 3 whatever those reads return: they are what
> the code reports once a committee concludes, and an integrator that omits them acts on an unaudited
> answer as if it were a confirmed one.
