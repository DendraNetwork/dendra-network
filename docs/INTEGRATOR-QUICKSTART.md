...
> ever been verified on-chain**, and `audit_state` is `"pending"` on every response today.
>
> Two gates stand between a job and a verdict, and **the first one stops the draw itself**. Under
> `committee_seed_source=1` the audit committee is drawn from a decentralised VRF seed; while
> `latest_contributors` stays under `committee_min_vrf_contributors`, **no committee is drawn at all** —
> the jobs are deferred and their fees stay held (`x/jobs/keeper/audit_sampling.go`, which logs
> `no decentralized VRF seed at this height -> NO audit draw`). Only past that does the second gate
> apply: a verdict needs `audit_min_quorum` jurors to vote, the jury excludes the miner under audit,
> and voting is gated on hardware — a registered miner that never runs `--judge` casts nothing.
> **Neither gate is a miner count**, so watching the miner count will not tell you when this changes;
> read `committee_seed_health` and `audit_min_quorum` from the chain.
>
> The `vindicated` / `rejected` branches of §3 have therefore never fired. Write them anyway: they are
> what the code will report once both gates clear, and an integrator that omits them acts on an
> unaudited answer as if it were a confirmed one.
