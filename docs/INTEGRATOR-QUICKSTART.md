...
> ever been verified on-chain**, and `audit_state` is `"pending"` in every response - always, by
> construction and not as a passing state: the gateway writes that field as a constant and never reads
> it back (see the note in section 2).
>
> **The reason sits upstream of the audit, and the audit gates are the wrong place to look: no job has
> ever been opened on this chain at all.** The `jobs` module's whole transaction history carries no
> `MsgOpenJob` - read it back with `tx_search` on `message.module='jobs'` and list the message types
> returned. Sampling, committee draw, jury and verdict are all downstream of a job existing, so none of
> them has ever had an input to act on.
>
> Two gates do stand between a job and a verdict once jobs flow, and **neither is a miner count**. The
> draw itself is gated on the seed: under `committee_seed_source=1` the audit committee is drawn from a
> decentralised VRF seed, and while `latest_contributors` stays under `committee_min_vrf_contributors`,
> **no committee is drawn at all** - the jobs are deferred and their fees stay held
> (`x/jobs/keeper/audit_sampling.go`, which logs `no decentralized VRF seed at this height -> NO audit
> draw`). Past it, a verdict needs `audit_min_quorum` jurors to vote, the jury excludes the miner under
> audit, and voting is gated on hardware - a registered miner that never runs `--judge` casts nothing.
> **Which gate is closed, if either, is a live value and not a property of the network**: read
> `committee_seed_health` and `audit_min_quorum` from the chain rather than assuming which one blocks.
>
> The `vindicated` / `rejected` branches of section 3 have therefore never fired. Write them anyway:
> they are what the code will report once jobs flow and both gates clear, and an integrator that omits
> them acts on an unaudited answer as if it were a confirmed one.
