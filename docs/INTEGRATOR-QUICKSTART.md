...
>
> **Set your client timeout above 300 s.** A single answer can take **over 200 seconds**: one miner GPU serves the whole network. A stock 30 s timeout fails every call, and it fails as a *client* timeout — the gateway never sees it, so no log on either side will tell you why.
