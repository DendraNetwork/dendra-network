...
# ⛔ THREE ANSWERS, NOT TWO. The two sections below publish into `site/` and read
# `site/_headers`. That tree is served from its own hosting and is deliberately NOT part of a
# published tree, so from a published clone it is absent. Run anyway, they would `mkdir` a
# directory this tree must not carry and then refuse with exit 6 -- reporting a DEFECT where
# there is only an ABSENCE, in a script wallet/README.md tells readers to run. What a third
# party came for is above and already done: the bundle is vendored, loads no CDN, and its
# SHA-256 is printed below. The refusal stays HARD wherever `site/` exists, because there a
# missing `_headers` is a real defect in a tree that should carry it.
if [ -d "$ROOT/site" ]; then
...
echo "   [OK] /wallet/ and /wallet/* name this page's script (LF and CRLF), with no token that undoes it"
  ...
  echo "## 6-7) publication under site/ and the /wallet/ policy guard -- [NOT MEASURED]"
  echo "   site/ is not in this tree. From a published clone that is EXPECTED: the deployed"
  echo "   root is served from its own hosting and carries its own headers. What this run DID"
  echo "   establish is above -- the bundle is vendored, loads no CDN, and its SHA-256 is"
  echo "   printed below, which is what makes it reproducible by a third party."
fi
