...
def _put(kind: str, key: str, data: bytes) -> None:
    ...
    with LOCK:
        ...
        # refused deposit is recoverable (the worker retries), erased evidence is not. Re-read here
        # rather than carried from the block above: a count decided under a lock that has since been
        # released is a count about the past.
...
class Handler(BaseHTTPRequestHandler):
    ...
    #     And `list` is not a monitoring nicety: it is THE work queue. `miner.py:688` opens its
    #     loop with `relay.listing()`, and no other call tells a miner a job is waiting for it. Behind
    #     a token, a joiner takes 401 on every round, `relay_client.py:149` returns `{}`, and the
    ...
    # TOKEN, and `dendra_parcours_etranger_garde.sh` counted the 401 as a PASS under the label "no
    ...
    def do_GET(self):
        ...
        if parts == ["list"]:
            ...
            # gating it behind a secret blinded every operator who did not have one.
