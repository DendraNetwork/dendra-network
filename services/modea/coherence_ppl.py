...
Dependencies: pip install transformers torch   (CPU is fine). The threshold comes from
ppl_calibrate.py, in the development repository: calibration tooling is excluded from
published trees by name, so the value is carried by DENDRA_PPL_THRESHOLD, not by a file here.
"""
...
def coherent(text, threshold=None):
    ...
    ppl_calibrate.py, in the development repository; default +inf, i.e. everything coherent
    while unset). INTENDED to replace
