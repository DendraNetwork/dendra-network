...
Two consequences, both in the code below:
  ...
  - `lecteur_job` (optional) resolves ONE job by its id through `GetJob`. That path cannot be
...
class JobRegistry(RegistryCache):
    ...
    `lecteur()` -> the JSON of `/dendra/jobs/v1/job` (the list).
    `lecteur_job(job_id)` -> the JSON of `/dendra/jobs/v1/job/{job_id}`. Optional but recommended:
        it is the only path pagination cannot truncate.
    """

    def __init__(self, lecteur, horloge, expiry: float = 300.0, lecteur_job=None):
        super().__init__(lecteur, horloge, expiry)
        if lecteur_job is not None and not callable(lecteur_job):
            raise ValueError("lecteur_job must be callable or None")
        self._lecteur_job = lecteur_job
    ...
    def _resoudre(self, job_id: str) -> bool:
        ...
        if self._lecteur_job is None:
            return False
        try:
            p = _paire(_charge(self._lecteur_job(job_id)).get("job"))
    ...
    def resume(self) -> str:
        ...
        return (f"JOB_REGISTRY state={self.state()} jobs={self.size()} "
                ...
                f"par_id={'yes' if self._lecteur_job else 'no'}")
