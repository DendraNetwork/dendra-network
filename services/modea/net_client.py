"""Mode A network client: talks to the miner server over HTTP.

The prompt is encrypted BEFORE it goes on the wire, and the **actual bytes of the network request** are
exposed so that the absence of plaintext can be proven rather than asserted.
...
class NetResult:
    output: str
    request_bytes: bytes      # what ACTUALLY went out on the network
    client_commit: str
    result_commit: str
