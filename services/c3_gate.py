...
def evaluate(state, cfg):
    ...
    add("held_lock_age", "warning", "WARN" if warn else "OK",
        ...
        "⚠ THESE TWO AGES ARE LOWER BOUNDS FOR THE FIRST BLOCKS AFTER A RESTART FROM A FRESH "
        "GENESIS, and the miss is on the QUIET side: ages travel through genesis as elapsed blocks "
        "and are re-anchored as `since = height - elapsed`, clamped at 0, so an age can never exceed "
        "the chain height. MEASURED: one fee exported at age 60 and read 29 blocks later returns 30 "
        "at height 1, 59 at height 30, and the faithful 89 only at height 200. An OK here therefore "
        "proves nothing while the height is still below the ages carried over; compare the age with "
        "the height before believing the silence. A uint64 anchor cannot express '60 blocks old' at "
        "height 1, so the loss lives in the state shape (ADR-045, entry 13 — arbitrated as "
        "accepted-and-declared rather than migrated).")
