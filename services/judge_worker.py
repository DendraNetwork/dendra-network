...
def verdict_already_posted(vkey: str, want: str) -> bool:
    ...
    """
    out = query("get-commit", vkey, flags=("--output", "json")) or ""
    try:
        d = json.loads(out)
    except Exception:
...
def revealed_prompt_matches(job_id, primary, rev, *, get_fields=None):
    ...
    THREE ANSWERS, AND THE THIRD IS WHY THIS IS NOT A BOOLEAN.
      ...
      None  -- UNKNOWABLE, and it stays an abstention. Two shapes reach it: a reveal with no
               `psalt`, from a primary whose build does not compute one; and an anchored prompt
               commitment EQUAL to the result commitment, which is what a build that cannot commit to
               the question anchors instead. Neither is evidence of anything, and reading either as
               False would slash honest miners for the age of their build.
    """
    ...
    # an operator noticed. Refusing a shape we cannot hash is an abstention, not an accusation: it says
    # nothing about the primary, only that there is nothing here to check.
    ...
    pc, rc = (f or {}).get("prompt") or "", (f or {}).get("result") or ""
    if not pc:
        return None                     # commit absent or unreadable: not a verdict
    if rc and pc == rc:
        return None                     # legacy duplicate: carries no prompt commitment
    return pc == rv.prompt_commitment(salt, question or "")
...
#
# ⛔ ONLY A HIGH COSINE IS INFORMATIVE, AND THAT IS NOT A DETAIL -- IT IS THE WHOLE SAFETY OF THIS GATE.
# `modea.miner._embed` picks its embedder from DENDRA_EMBED_MODE, a NODE-LOCAL environment variable,
# never from the model-id the commit declares. Its own docstring names the trap: a hash vector compared
# against sentence-transformer vectors yields a cosine near zero. A LOW cosine is therefore
# indistinguishable between "the primary revealed something else" (cheating) and "these two operators
# configured different embedders" (a config mismatch) -- and slashing on that ambiguity would punish an
# honest, misconfigured miner. Low cosine ABSTAINS. High cosine proves the reveal IS the anchored work,
# which is the only thing this gate is asked to establish.
#
# The bar is an IDENTITY bar, close to 1: same text, same embedder. It is deliberately NOT the chain's
# `semantic_threshold_bps`, which answers a different question (are two ANSWERS the same fact) and
# would be a number borrowed from another purpose.
ANCHOR_IDENTITY_COS = 0.98


def reveal_matches_anchor(job_id, primary, answer, *, get_root=None, embed=None):
    """Cosine between the revealed answer and the commit the primary anchored, or None when unknowable.

    None is an ABSTENTION, never a pass and never a slash: an absent commit, an unreadable one, an
    embedder that will not run, or two vectors of different length (different embedders) are all states
    in which this judge cannot tell cheating from configuration."""
    root = (get_root or anchored_root)(f"{job_id}__{primary}")
    if not root:
        return None
    try:
        if embed is None:
            from modea.miner import _embed as embed
        mine = embed(answer)
    except Exception:
        return None
    try:
            ...
            for job_id, primary, dispute_h, rev, _now in observed:
                ...
                if not rev or "prompt" not in rev:
                    ...
                    if verdict_already_posted(vkey, "0"):
                        ...
                        continue
                    out = tx_from(a.id, "create-commit", vkey, "0", "0", "verdict", flags=flags)
                    if wait_tx(out):
                    ...
                    continue
                slow.append((job_id, primary, rev))
            for job_id, primary, rev in slow:
                ...
                # CONFRONT THE REVEAL WITH WHAT THE PRIMARY ANCHORED, BEFORE JUDGING ITS CONTENT.
                # Judging first and checking later would be the same hole: the expensive stages would
                # have scored a text nobody committed to.
                # Confronts the ANSWER only. The PROMPT below comes from the same reveal, i.e. from
                # the audited party, and nothing anchors it -- see the note at the head of
                # `decide_verdict`. A high cosine proves the revealed answer is the one committed on
                # chain; it proves nothing about the question it was committed FOR.
                # THE QUESTION FIRST: grading an answer against a question the audited party chose
                # measures nothing at all, so a proven mismatch ends the job before any generation.
                #
                # AND IT ABSTAINS RATHER THAN VOTING TO SLASH, which is a deliberate choice and not an
                # oversight. A mismatch is strong evidence -- only the holder of the miner key can
                # compute the salt, so only the primary could have produced the anchored value -- but
                # it is not the ONLY path to a mismatch: a primary that rotates its reveal key between
                # the commit and the reveal derives a different salt and fails this check while having
                # cheated at nothing. Until this signal has field evidence, refusing to certify costs
                # a cheater its fee; voting to slash on it would cost an honest operator its stake for
                # an operational act. Withholding a certificate is reversible, a slash is not.
                _pm = revealed_prompt_matches(job_id, primary, rev)
                if _pm is False:
                    print(f"[judge] {a.id} {job_id}: the revealed QUESTION does not match the one "
                          f"{primary} committed on chain -- refusing to grade an answer against a "
                          f"question chosen after the fact. ABSTAINING.", flush=True)
                    continue
                _cos = reveal_matches_anchor(job_id, primary, rev["answer"])
                if _cos is None or _cos < ANCHOR_IDENTITY_COS:
                    _why = "no readable anchor" if _cos is None else f"cos={_cos:.3f} < {ANCHOR_IDENTITY_COS}"
                    print(f"[judge] {a.id} {job_id}: reveal does not match the anchored commit "
                          f"({_why}) -> ABSTAIN (this is not a slash: a low cosine is also what two "
                          f"different DENDRA_EMBED_MODE settings produce)")
                    tr["stage"] = "anchor-mismatch"
                    verdict = None
                else:
                ...
                if verdict_already_posted(vkey, v):
                    ...
                    continue
                out = tx_from(a.id, "create-commit", vkey, v, v, "verdict", flags=flags)
                if wait_tx(out):
