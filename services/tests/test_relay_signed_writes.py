...
#
# ⛔ ATTRIBUTION READS THE KEY, AND FOR `reveal` THE KEY NAMES THE WRONG PARTY. `_miner_of_key` takes
# the segment after the last `__`, which is right for `res/<jid>__<mid>` (the miner that answers) and
# for `pub`/`attest` (the key IS the identifier). A reveal is keyed `<jid>__<recipient>`
# (`reveal_helpers.py` seals one copy per committee member), so the segment names the JUROR the copy is
# addressed to — while the writer is the PRIMARY.
#
# MEASURED against the shipped server in `enforce`: the primary signing its own reveal gets 401; the
# recipient signing the very same deposit gets 200. The signature path therefore demands the key of the
# party that is supposed to READ the artifact.
#
# ⚠️ NOT A LIVE OUTAGE, AND SAYING OTHERWISE WOULD BE THE OVERCLAIM. `DENDRA_RELAY_SIGN` defaults to
# `off` and nothing under `deploy/` or `docker/` sets it, so today the token gates these writes and
# honest deposits pass. What this pins is a trap laid on the exact upgrade path the design plans: the
# per-route policy exists so a joiner without the shared secret can still work, and its own comment
# says "`reveal` matters most: it is the primary's obligation, and a sampled job with no reveal has no
# artifact to judge, so its held fee never resolves". Turning `enforce` on would strand precisely that.
#
# ⛔ PINNED, NOT FIXED — the fix needs knowledge the relay does not have. Attributing a reveal to its
# writer means either changing the key shape (and every reader that fetches `<jid>__<my_id>`), or
# learning from the chain WHICH miner is the primary of that job. Accepting "any registered miner"
# instead would let a stranger occupy a reveal key before the primary does, and write-once would then
# lock the honest deposit out — trading a refusal for a squat. When it lands, the first case flips.

def _reveal_identities():
    prim = _identity()
    juror = _identity()
    return prim, juror


def test_enforce_ACCEPTS_the_primary_that_writes_its_own_reveal(monkeypatch=None):
    """ADR-045 (14). A reveal key now names its AUTHOR as the LAST segment, so the relay attribution
    -- the segment after the last `__` -- finally designates the party that writes.

    Before, the key was `<jid>__<recipient>`: SIGNED by the primary, KEYED by its reader. The primary
    signing its own reveal got 401, and the recipient signing the SAME deposit got 200 -- the exact
    inverse of what is needed. Inert while `DENDRA_RELAY_SIGN` is `off`, but it was a trap laid on the
    upgrade path the per-route policy exists to open."""
    (sk_p, pub_p, addr_p), (_, _, addr_j) = _reveal_identities()
    registry_ = {"dm1primary": addr_p, "dm1juror": addr_j}
    key_ = "j1__dm1juror__dm1primary"   # sealed FOR the juror, written BY the primary, which ends it
    with Relay("enforce", registry=registry_) as r:
        code, _ = r.deposit(kind="reveal", key=key_,
                            headers=_headers(sk_p, pub_p, addr_p, kind="reveal", key=key_,
                                             miner="dm1primary"))
        assert code == 200, "the author of a reveal must be able to deposit it through the signed path"


def test_enforce_REFUSES_the_recipient_that_writes_someone_elses_reveal():
    """The counterpart, and the one that makes the fix USEFUL rather than merely different: under the
    old shape the RECIPIENT could deposit -- hence squat the slot the primary had to fill, which leaves
    an audit with no judgeable reveal. It no longer can."""
    (_, _, addr_p), (sk_j, pub_j, addr_j) = _reveal_identities()
    registry_ = {"dm1primary": addr_p, "dm1juror": addr_j}
    key_ = "j1__dm1juror__dm1primary"
    with Relay("enforce", registry=registry_) as r:
        code, _ = r.deposit(kind="reveal", key=key_,
                            headers=_headers(sk_j, pub_j, addr_j, kind="reveal", key=key_,
                                             miner="dm1juror"))
        assert code == 401, "the recipient is not the author: it does not fill the slot"

def test_the_attribution_rule_is_UNIFORM_across_kinds():
    """THE INVARIANT, pinned once for every kind: the LAST segment is the AUTHOR.

    This case replaces the one showing the recipient accepted on a reveal under `<jid>__<mid>`. That
    shape is no longer written by anything since ADR-045 (14), and a case pinning a state the code can
    no longer reach measures nothing: it freezes a fixture, not a behaviour.
    What deserves holding is that the rule has NO per-kind exception -- a rule without exceptions
    cannot forget one, and that is the whole gain of the shape change."""
    (sk_p, pub_p, addr_p), (_, _, addr_j) = _reveal_identities()
    registry_ = {"dm1primary": addr_p, "dm1juror": addr_j}
    with Relay("enforce", registry=registry_) as r:
        for kind, key_ in (("res", "j1__dm1primary"),
                          ("reveal", "j1__dm1juror__dm1primary")):
            code, _ = r.deposit(kind=kind, key=key_,
                                headers=_headers(sk_p, pub_p, addr_p, kind=kind, key=key_,
                                                 miner="dm1primary"))
            assert code == 200, (
                "%s: the last segment names the author, so the author deposits" % kind)

def test_the_same_writer_is_accepted_on_a_kind_whose_key_names_it():
    """THE CONTROL. Identical identity, identical signing code, on `res/<jid>__<mid>` where the key
    segment IS the writer: accepted. Without this the two cases above could be read as a broken
    signature path rather than a mismatched key shape."""
    sk, pub, address = _identity()
    with Relay("enforce", registry={"dm1abc": address}) as r:
        assert r.deposit(headers=_headers(sk, pub, address))[0] == 200
