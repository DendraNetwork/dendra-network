"""Mode A prototype tests. Run: pytest -q (from services/)."""
import json
import sys
...
def test_canary_traces_leak():
    ...
    leaked = canary_mod.embed("a leaking prompt", can)
    assert reg.detect(leaked) == ["miner-XYZ"]
    assert reg.detect("text carrying no canary") == []


def test_canary_stripped_from_output():
    can = canary_mod.make_canary()
    txt = canary_mod.embed("answer", can)
