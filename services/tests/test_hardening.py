"""Hardening tests: secure memory, core-dump suppression, software attestation."""
import sys
...
def test_securebytes_roundtrip_and_zeroize():
    ...
    sb.close()
    assert all(b == 0 for b in sb.buf)          # zeroized after close
...
def test_self_attest_detects_tamper():
    manifest = build_manifest()
    # simulate a tampered binary by falsifying the expected hash of one file
