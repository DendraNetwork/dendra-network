...
def test_the_store_key_follows_the_PROOF():
    ...
    `machine` and `node_id` are chosen by the sender at both ends: a hundred reports, a hundred
    entries, all naming the same staked miner. A proven identity occupies ONE key -- duplicating then
    costs as many operator keys as entries wanted, which is precisely the price this was meant to set.

    ⛔ AND THE TWO KEYSPACES MUST NOT MEET, which is what this case failed to say. It used to pin the
    declared key as the literal `box-nom-choisi::nom-choisi` -- spelling the defect out without
    naming it. The declared form carried NO prefix of its own, so a sender could set `machine` to
    "signe" and `node_id` to a staked miner id and land on `signe::<miner_id>`: the key of a PROVEN
    record, which `do_POST` then overwrote with an unsigned one, emptying the `verified` block that
    public surfaces display and that the chat gate refuses to open below. The exact literal was
    incidental; what this case exists for is that neither namespace can spell the other."""
    raw = _report("nom-choisi", [REGISTERED])
    proven_key = cs.storage_key(_proven(raw, REGISTERED))

    # ⚠️ THE EVICTION IS ASSERTED FIRST, AND AS A PROPERTY RATHER THAN A LITERAL. Against a module
    # that still shares one keyspace this case has to fail on the CLAIM; ordered the other way it
    # failed on a missing module attribute instead, which reads as "this test is out of date" and
    # says nothing about what the server does. Naming a constant is not measuring a behaviour.
    for machine in ("signe", "decl", "", "signe::x", "box-1"):
        forged = _report(REGISTERED, [])
        forged["machine"] = machine
        assert cs.storage_key(forged) != proven_key, (
            "an unsigned deposit must not be able to name, and therefore replace, a proven record; "
            "machine=%r reached it" % machine)

    declared = cs.storage_key(raw)
    assert "nom-choisi" in declared, "the declared key still identifies what the sender declared"
    hundred = {cs.storage_key(_report("nom-%d" % i, [REGISTERED])) for i in range(100)}
    assert len(hundred) == 100, "a hundred chosen names, a hundred entries: the escalation as it stands"

    proven_set = {cs.storage_key(_proven(_report("nom-%d" % i, [REGISTERED]), REGISTERED))
               for i in range(100)}
    assert proven_set == {proven_key}, (
        "a hundred reports PROVEN by the same identity occupy ONE key, not a hundred")

    # And retiring the pre-namespace key must never hand that reach back one level up: `machine`
    # set to a namespace this file owns turns the retired key into a CURRENT one.
    for machine in ("signe", "decl"):
        f = _report("abc123::n1", [])
        f["machine"] = machine
        assert cs.superseded_keys(f) == (), "machine=%r retired a live key" % machine
