"""Persistent ledger, reputation accounting, and coordinator liveness."""
import sys
import time
...
def test_ledger_persists_only_hashes(tmp_path):
    ...
    assert "4471" not in text and "prompt" not in text  # no content, only hashes
...
def test_registry_liveness_drop():
    ...
    time.sleep(0.1)
    assert reg.healthy() == []            # expires without a heartbeat
    reg.heartbeat("m1")
    assert len(reg.healthy()) == 1        # comes back after one heartbeat
