...
print()
print("== THE WORK QUEUE IS OPEN; WRITING IT IS NOT ==")
# ⛔ THIS CASE USED TO EXPECT `TOKEN`, AND THAT EXPECTATION CERTIFIED THE DEFECT FOR NINE DAYS.
# `GET /list` is the ONLY route that tells a miner a job is waiting for it (`miner.py:688`
# opens its loop with it, and nothing else carries that signal). Behind the token, an outside
# operator took 401 on every round, `relay_client.py:149` returned `{}`, and the daemon printed
# "waiting for jobs" forever. An operator reported exactly that on 2026-08-08; this line is why
# nobody here saw it. A test that encodes the wall as the right answer stays green and protects
# the wall — the shape ADR-032 already had to undo once, in the sybil test.
# The old rationale was "list maps who processes what". The chain publishes that map itself, to
# anyone, with no secret: `/dendra/jobs/v1/job` carries job_id + miner_id, and
# `/dendra/jobs/v1/assigned_committee/<jobId>` the drawn members. Measured anonymously 2026-08-12.
case("GET  list  -> OPEN (the work queue)", policy(["list"], "GET"), OPEN)
case("POST list  -> token (reading it is not writing it)", policy(["list"], "POST"), TOKEN)
case("GET  stats -> token (nothing depends on it)", policy(["stats"], "GET"), TOKEN)
...
print()
print("== THE JOINER'S ACTUAL LOOP, AGAINST A REAL RELAY WITH A TOKEN CONFIGURED ==")
# ⛔ THE TABLE ABOVE CANNOT ASK THIS QUESTION. It exercises `_politique`, a pure function; it says
# nothing about whether `do_GET` honours the answer. A policy that returns OPEN while the handler
# guards the route separately would keep every table case green — the exact blindness that let a
# private-key guard die in July with 111/111 green. So this block SPEAKS HTTP, with a token
# CONFIGURED on the relay and NEVER sent by the caller: the position of an outside operator.
with Relay("observe", REGISTERED, token="t" * 32) as r:
    case("GET /list with no token -> 200 (a miner can see its work)",
         r._call("GET", "list"), 200)
    # A 200 alone would also be returned by an empty stub. Read the body and require the queue's
    # shape: the point is that the miner SEES its work, not that the route answers.
    with urllib.request.urlopen(f"http://127.0.0.1:{r.port}/list", timeout=5) as _r:
        _corps = _r.read().decode()
    case("...and the body really carries the queue", 200 if _corps.startswith("{") and '"res"' in _corps else 0, 200)
    case("POST /list with no token -> 401 (reading is not writing)",
         r._call("POST", "list", b"{}"), 401)
    case("GET /stats with no token -> 401 (nothing depends on it)",
         r._call("GET", "stats"), 401)
