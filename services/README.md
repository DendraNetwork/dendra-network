...
## Who runs what — the two sides, and the seam between them

This folder holds **both halves** of the network. Which half a file belongs to decides who is affected
when it breaks, so it is worth knowing before you read further.

| Component | Runs on | |
|-----------|---------|--|
| `miner.py`, the judge and reveal workers | **the operator's machine** | started inside the miner container by `deploy/join.sh` |
| `capacity_sign.py` | **the operator's machine** | signs the hardware report with the miner's own key |
| `client.py`, `cli.py` | **whoever calls the network** | |
| `relay.py`, `capacity_server.py`, `faucet.py`, `gateway.py` | **the network operator's host** | reached over HTTP; you never run these to join |
| **`modea/`** | **both sides** | the shared protocol |

⚠️ **`modea/` is the seam, and it is where a mismatch hides.** A signature is produced on one side and
verified on the other; the canonical message, the key encoding and the address derivation must agree
exactly. Each half can be correct on its own while the pair does not work — and nothing fails loudly,
because the verifying side is written to answer *"I cannot believe this"* rather than to raise.

⇒ **A change in `modea/`, or in either side of a pair that meets there, needs a test that exercises the
PAIR** — one that signs with the operator's code and verifies with the network's. A test that re-implements
either side proves only that the test agrees with itself.
...
pytest -q tests/       # full suite: crypto, MPC, coordinator, network, persistence, hardening, chain bridge
```
